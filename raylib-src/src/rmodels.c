...
static Model RLLoadOBJ(const char *fileName);     // Load OBJ mesh data
#endif
#if defined(SUPPORT_FILEFORMAT_IQM)
static Model RLLoadIQM(const char *fileName);     // Load IQM mesh data
static ModelAnimation *RLLoadModelAnimationsIQM(const char *fileName, int *animCount);   // Load IQM animation data
#endif
#if defined(SUPPORT_FILEFORMAT_GLTF)
static Model RLLoadGLTF(const char *fileName);    // Load GLTF mesh data
static ModelAnimation *RLLoadModelAnimationsGLTF(const char *fileName, int *animCount);  // Load GLTF animation data
#endif
#if defined(SUPPORT_FILEFORMAT_VOX)
static Model RLLoadVOX(const char *filename);     // Load VOX mesh data
#endif
#if defined(SUPPORT_FILEFORMAT_M3D)
static Model RLLoadM3D(const char *filename);     // Load M3D mesh data
static ModelAnimation * RLLoadModelAnimationsM3D(const char *fileName, int *animCount);   // Load M3D animation data
#endif
#if defined(SUPPORT_FILEFORMAT_OBJ) || defined(SUPPORT_FILEFORMAT_MTL)
static void RLProcessMaterialsOBJ(Material *rayMaterials, tinyobj_material_t *materials, int materialCount);  // Process obj materials
#endif
...
void RLDrawLine3D(Vector3 startPos, Vector3 endPos, Color color)
{
    rlBegin(RL_LINES);
...
void RLDrawPoint3D(Vector3 position, Color color)
{
    rlPushMatrix();
...
void RLDrawCircle3D(Vector3 center, float radius, Vector3 rotationAxis, float rotationAngle, Color color)
{
    rlPushMatrix();
...
void RLDrawTriangle3D(Vector3 v1, Vector3 v2, Vector3 v3, Color color)
{
...
void RLDrawTriangleStrip3D(const Vector3 *points, int pointCount, Color color)
{
...
void RLDrawCube(Vector3 position, float width, float height, float length, Color color)
{
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
...
void RLDrawCubeV(Vector3 position, Vector3 size, Color color)
{
    RLDrawCube(position, size.x, size.y, size.z, color);
}

// Draw cube wires
void RLDrawCubeWires(Vector3 position, float width, float height, float length, Color color)
{
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
...
void RLDrawCubeWiresV(Vector3 position, Vector3 size, Color color)
{
    RLDrawCubeWires(position, size.x, size.y, size.z, color);
}

// Draw sphere
void RLDrawSphere(Vector3 centerPos, float radius, Color color)
{
    RLDrawSphereEx(centerPos, radius, 16, 16, color);
}

// Draw sphere with extended parameters
void RLDrawSphereEx(Vector3 centerPos, float radius, int rings, int slices, Color color)
{
#if 0
...
// Draw sphere wires
void RLDrawSphereWires(Vector3 centerPos, float radius, int rings, int slices, Color color)
{
    rlPushMatrix();
        // NOTE: Transformation is applied in inverse order (scale -> translate)
        rlTranslatef(centerPos.x, centerPos.y, centerPos.z);
...
void RLDrawCylinder(Vector3 position, float radiusTop, float radiusBottom, float height, int sides, Color color)
{
    if (sides < 3) sides = 3;
...
// NOTE: It could be also used for pyramid and cone
void RLDrawCylinderEx(Vector3 startPos, Vector3 endPos, float startRadius, float endRadius, int sides, Color color)
{
    if (sides < 3) sides = 3;
    ...
    Vector3 b1 = RLVector3Normalize(RLVector3Perpendicular(direction));
    Vector3 b2 = RLVector3Normalize(RLVector3CrossProduct(b1, direction));
...
// NOTE: It could be also used for pyramid and cone
void RLDrawCylinderWires(Vector3 position, float radiusTop, float radiusBottom, float height, int sides, Color color)
{
    if (sides < 3) sides = 3;
...
// NOTE: It could be also used for pyramid and cone
void RLDrawCylinderWiresEx(Vector3 startPos, Vector3 endPos, float startRadius, float endRadius, int sides, Color color)
{
    if (sides < 3) sides = 3;
...
void RLDrawCapsule(Vector3 startPos, Vector3 endPos, float radius, int slices, int rings, Color color)
{
    ...
    Vector3 b0 = RLVector3Normalize(direction);
    Vector3 b1 = RLVector3Normalize(RLVector3Perpendicular(direction));
    Vector3 b2 = RLVector3Normalize(RLVector3CrossProduct(b1, direction));
...
void RLDrawCapsuleWires(Vector3 startPos, Vector3 endPos, float radius, int slices, int rings, Color color)
{
    if (slices < 3) slices = 3;
...
// Draw a plane
void RLDrawPlane(Vector3 centerPos, Vector2 size, Color color)
{
...
// Draw a ray line
void RLDrawRay(Ray ray, Color color)
{
    float scale = 10000;
...
void RLDrawGrid(int slices, float spacing)
{
...
Model RLLoadModel(const char *fileName)
{
    Model model = { 0 };

#if defined(SUPPORT_FILEFORMAT_OBJ)
    if (RLIsFileExtension(fileName, ".obj")) model = RLLoadOBJ(fileName);
#endif
#if defined(SUPPORT_FILEFORMAT_IQM)
    if (RLIsFileExtension(fileName, ".iqm")) model = RLLoadIQM(fileName);
#endif
#if defined(SUPPORT_FILEFORMAT_GLTF)
    if (RLIsFileExtension(fileName, ".gltf") || RLIsFileExtension(fileName, ".glb")) model = RLLoadGLTF(fileName);
#endif
#if defined(SUPPORT_FILEFORMAT_VOX)
    if (RLIsFileExtension(fileName, ".vox")) model = RLLoadVOX(fileName);
#endif
#if defined(SUPPORT_FILEFORMAT_M3D)
    if (RLIsFileExtension(fileName, ".m3d")) model = RLLoadM3D(fileName);
#endif

    // Make sure model transform is set to identity matrix!
    model.transform = RLMatrixIdentity();

    if ((model.meshCount != 0) && (model.meshes != NULL))
    {
        // Upload vertex data to GPU (static meshes)
        for (int i = 0; i < model.meshCount; i++) RLUploadMesh(&model.meshes[i], false);
    }
    ...
    if (model.materialCount == 0)
    {
        ...
        model.materials[0] = RLLoadMaterialDefault();
...
Model RLLoadModelFromMesh(Mesh mesh)
{
    Model model = { 0 };
...
bool RLIsModelValid(Model model)
{
    bool result = false;
...
void RLUnloadModel(Model model)
{
    // Unload meshes
...
BoundingBox RLGetModelBoundingBox(Model model)
{
    ...
    if (model.meshCount > 0)
    {
        ...
        {
            BoundingBox tempBounds = RLGetMeshBoundingBox(model.meshes[i]);
    ...
    bounds.min = RLVector3Transform(bounds.min, model.transform);
    bounds.max = RLVector3Transform(bounds.max, model.transform);

    return bounds;
}

// Upload vertex data into a VAO (if supported) and VBO
void RLUploadMesh(Mesh *mesh, bool dynamic)
{
    if (mesh->vaoId > 0)
    {
...
void RLUpdateMeshBuffer(Mesh mesh, int index, const void *data, int dataSize, int offset)
{
...
void RLDrawMesh(Mesh mesh, Material material, Matrix transform)
{
    ...
    Matrix matModel = RLMatrixIdentity();
    Matrix matView = rlGetMatrixModelview();
    Matrix matModelView = RLMatrixIdentity();
    ...
    matModel = RLMatrixMultiply(transform, rlGetMatrixTransform());
    ...
    matModelView = RLMatrixMultiply(matModel, matView);

    // Upload model normal matrix (if locations available)
    if (material.shader.locs[SHADER_LOC_MATRIX_NORMAL] != -1) rlSetUniformMatrix(material.shader.locs[SHADER_LOC_MATRIX_NORMAL], RLMatrixTranspose(RLMatrixInvert(matModel)));
    ...
    for (int eye = 0; eye < eyeCount; eye++)
    {
        ...
        Matrix matModelViewProjection = RLMatrixIdentity();
        if (eyeCount == 1) matModelViewProjection = RLMatrixMultiply(matModelView, matProjection);
        else
        {
            // Setup current eye viewport (half screen width)
            rlViewport(eye*rlGetFramebufferWidth()/2, 0, rlGetFramebufferWidth()/2, rlGetFramebufferHeight());
            matModelViewProjection = RLMatrixMultiply(RLMatrixMultiply(matModelView, rlGetMatrixViewOffsetStereo(eye)), rlGetMatrixProjectionStereo(eye));
        }
...
void RLDrawMeshInstanced(Mesh mesh, Material material, const Matrix *transforms, int instances)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    ...
    for (int i = 0; i < instances; i++) instanceTransforms[i] = RLMatrixToFloatV(transforms[i]);
    ...
    matModelView = RLMatrixMultiply(rlGetMatrixTransform(), matView);
...
bool RLExportMesh(Mesh mesh, const char *fileName)
{
    bool success = false;

    if (RLIsFileExtension(fileName, ".obj"))
    {
...
bool RLExportMeshAsCode(Mesh mesh, const char *fileName)
{
    bool success = false;
    ...
    strcpy(varFileName, RLGetFileNameWithoutExt(fileName));
...
static void RLProcessMaterialsOBJ(Material *materials, tinyobj_material_t *mats, int materialCount)
{
    // Init model mats
    for (int m = 0; m < materialCount; m++)
    {
        ...
        materials[m] = RLLoadMaterialDefault();
        ...
        if (mats[m].diffuse_texname != NULL) materials[m].maps[MATERIAL_MAP_DIFFUSE].texture = RLLoadTexture(mats[m].diffuse_texname);  //char *diffuse_texname; // map_Kd
        ...
        if (mats[m].specular_texname != NULL) materials[m].maps[MATERIAL_MAP_SPECULAR].texture = RLLoadTexture(mats[m].specular_texname);  //char *specular_texname; // map_Ks
        ...
        if (mats[m].bump_texname != NULL) materials[m].maps[MATERIAL_MAP_NORMAL].texture = RLLoadTexture(mats[m].bump_texname);  //char *bump_texname; // map_bump, bump
        ...
        if (mats[m].displacement_texname != NULL) materials[m].maps[MATERIAL_MAP_HEIGHT].texture = RLLoadTexture(mats[m].displacement_texname);  //char *displacement_texname; // disp
    }
}
#endif

// Load materials from model file
Material * RLLoadMaterials(const char *fileName, int *materialCount)
{
    ...
    if (RLIsFileExtension(fileName, ".mtl"))
    {
        ...
        RLProcessMaterialsOBJ(materials, mats, count);
...
Material RLLoadMaterialDefault(void)
{
    Material material = { 0 };
...
bool RLIsMaterialValid(Material material)
{
    bool result = false;
...
void RLUnloadMaterial(Material material)
{
...
void RLSetMaterialTexture(Material *material, int mapType, Texture2D texture)
{
    material->maps[mapType].texture = texture;
}

// Set the material for a mesh
void RLSetModelMeshMaterial(Model *model, int meshId, int materialId)
{
...
ModelAnimation * RLLoadModelAnimations(const char *fileName, int *animCount)
{
    ModelAnimation *animations = NULL;

#if defined(SUPPORT_FILEFORMAT_IQM)
    if (RLIsFileExtension(fileName, ".iqm")) animations = RLLoadModelAnimationsIQM(fileName, animCount);
#endif
#if defined(SUPPORT_FILEFORMAT_M3D)
    if (RLIsFileExtension(fileName, ".m3d")) animations = RLLoadModelAnimationsM3D(fileName, animCount);
#endif
#if defined(SUPPORT_FILEFORMAT_GLTF)
    if (RLIsFileExtension(fileName, ".gltf;.glb")) animations = RLLoadModelAnimationsGLTF(fileName, animCount);
#endif
...
void RLUpdateModelAnimationBones(Model model, ModelAnimation anim, int frame)
{
        ...
        for (int i = 0; i < model.meshCount; i++)
        {
            if (model.meshes[i].boneMatrices)
            {
                ...
                for (int boneId = 0; boneId < model.meshes[i].boneCount; boneId++)
                {
                    ...
                    Vector3 invTranslation = RLVector3RotateByQuaternion(RLVector3Negate(inTranslation), RLQuaternionInvert(inRotation));
                    Quaternion invRotation = RLQuaternionInvert(inRotation);
                    Vector3 invScale = RLVector3Divide((Vector3){ 1.0f, 1.0f, 1.0f }, inScale);

                    Vector3 boneTranslation = RLVector3Add(
                        RLVector3RotateByQuaternion(RLVector3Multiply(outScale, invTranslation),
                        outRotation), outTranslation);
                    Quaternion boneRotation = RLQuaternionMultiply(outRotation, invRotation);
                    Vector3 boneScale = RLVector3Multiply(outScale, invScale);

                    Matrix boneMatrix = RLMatrixMultiply(RLMatrixMultiply(
                        RLQuaternionToMatrix(boneRotation),
                        RLMatrixTranslate(boneTranslation.x, boneTranslation.y, boneTranslation.z)),
                        RLMatrixScale(boneScale.x, boneScale.y, boneScale.z));
...
void RLUpdateModelAnimation(Model model, ModelAnimation anim, int frame)
{
    RLUpdateModelAnimationBones(model,anim,frame);
    for (int m = 0; m < model.meshCount; m++)
    {
        ...
        for (int vCounter = 0; vCounter < vValues; vCounter += 3)
        {
            ...
            for (int j = 0; j < 4; j++, boneCounter++)
            {
                ...
                animVertex = RLVector3Transform(animVertex,model.meshes[m].boneMatrices[boneId]);
                ...
                if (mesh.normals != NULL)
                {
                    ...
                    animNormal = RLVector3Transform(animNormal,model.meshes[m].boneMatrices[boneId]);
...
void RLUnloadModelAnimations(ModelAnimation *animations, int animCount)
{
    for (int i = 0; i < animCount; i++) RLUnloadModelAnimation(animations[i]);
    RL_FREE(animations);
}

// Unload animation data
void RLUnloadModelAnimation(ModelAnimation anim)
{
...
bool RLIsModelAnimationValid(Model model, ModelAnimation anim)
{
    int result = true;
...
Mesh RLGenMeshPoly(int sides, float radius)
{
    Mesh mesh = { 0 };
...
Mesh RLGenMeshPlane(float width, float length, int resX, int resZ)
{
    Mesh mesh = { 0 };
...
Mesh RLGenMeshCube(float width, float height, float length)
{
    Mesh mesh = { 0 };
...
Mesh RLGenMeshSphere(float radius, int rings, int slices)
{
    Mesh mesh = { 0 };
...
Mesh RLGenMeshHemiSphere(float radius, int rings, int slices)
{
    Mesh mesh = { 0 };
...
Mesh RLGenMeshCylinder(float radius, float height, int slices)
{
    Mesh mesh = { 0 };
...
Mesh RLGenMeshCone(float radius, float height, int slices)
{
    Mesh mesh = { 0 };
...
// Generate torus mesh
Mesh RLGenMeshTorus(float radius, float size, int radSeg, int sides)
{
    Mesh mesh = { 0 };
...
Mesh RLGenMeshKnot(float radius, float size, int radSeg, int sides)
{
    Mesh mesh = { 0 };
...
Mesh RLGenMeshHeightmap(Image heightmap, Vector3 size)
{
    ...
    Color *pixels = RLLoadImageColors(heightmap);
    ...
    for (int z = 0; z < mapZ-1; z++)
    {
        for (int x = 0; x < mapX-1; x++)
        {
            ...
            for (int i = 0; i < 18; i += 9)
            {
                ...
                vN = RLVector3Normalize(RLVector3CrossProduct(RLVector3Subtract(vB, vA), RLVector3Subtract(vC, vA)));
    ...
    RLUnloadImageColors(pixels);  // Unload pixels color data
...
// NOTE: Vertex data is uploaded to GPU
Mesh RLGenMeshCubicmap(Image cubicmap, Vector3 cubeSize)
{
    #define COLOR_EQUAL(col1, col2) ((col1.r == col2.r)&&(col1.g == col2.g)&&(col1.b == col2.b)&&(col1.a == col2.a))

    Mesh mesh = { 0 };

    Color *pixels = RLLoadImageColors(cubicmap);
...
BoundingBox RLGetMeshBoundingBox(Mesh mesh)
{
    ...
    if (mesh.vertices != NULL)
    {
        ...
        {
            minVertex = RLVector3Min(minVertex, (Vector3){ mesh.vertices[i*3], mesh.vertices[i*3 + 1], mesh.vertices[i*3 + 2] });
            maxVertex = RLVector3Max(maxVertex, (Vector3){ mesh.vertices[i*3], mesh.vertices[i*3 + 1], mesh.vertices[i*3 + 2] });
        }
    }
...
void RLGenMeshTangents(Mesh *mesh)
{
    ...
    for (int i = 0; i < mesh->vertexCount; i++)
    {
        ...
        Vector3 tmp = RLVector3Subtract(tangent, Vector3Scale(normal, Vector3DotProduct(normal, tangent)));
...
#else
        RLVector3OrthoNormalize(&normal, &tangent);
        mesh->tangents[i*4 + 0] = tangent.x;
        mesh->tangents[i*4 + 1] = tangent.y;
        mesh->tangents[i*4 + 2] = tangent.z;
        mesh->tangents[i*4 + 3] = (RLVector3DotProduct(RLVector3CrossProduct(normal, tangent), tan2[i]) < 0.0f)? -1.0f : 1.0f;
#endif
    }
...
void RLDrawModel(Model model, Vector3 position, float scale, Color tint)
{
    Vector3 vScale = { scale, scale, scale };
    Vector3 rotationAxis = { 0.0f, 1.0f, 0.0f };

    RLDrawModelEx(model, position, rotationAxis, 0.0f, vScale, tint);
}

// Draw a model with extended parameters
void RLDrawModelEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint)
{
    // Calculate transformation matrix from function parameters
    // Get transform matrix (rotation -> scale -> translation)
    Matrix matScale = RLMatrixScale(scale.x, scale.y, scale.z);
    Matrix matRotation = RLMatrixRotate(rotationAxis, rotationAngle*DEG2RAD);
    Matrix matTranslation = RLMatrixTranslate(position.x, position.y, position.z);

    Matrix matTransform = RLMatrixMultiply(RLMatrixMultiply(matScale, matRotation), matTranslation);

    // Combine model transformation matrix (model.transform) with matrix generated by function parameters (matTransform)
    model.transform = RLMatrixMultiply(model.transform, matTransform);
...
void RLDrawModelWires(Model model, Vector3 position, float scale, Color tint)
{
    rlEnableWireMode();

    RLDrawModel(model, position, scale, tint);

    rlDisableWireMode();
}

// Draw a model wires (with texture if set) with extended parameters
void RLDrawModelWiresEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint)
{
    rlEnableWireMode();

    RLDrawModelEx(model, position, rotationAxis, rotationAngle, scale, tint);

    rlDisableWireMode();
}

// Draw a model points
void RLDrawModelPoints(Model model, Vector3 position, float scale, Color tint)
{
    rlEnablePointMode();
...
// Draw a model points
void RLDrawModelPointsEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint)
{
    rlEnablePointMode();
    rlDisableBackfaceCulling();
...
// Draw a billboard
void RLDrawBillboard(Camera camera, Texture2D texture, Vector3 position, float scale, Color tint)
{
    Rectangle source = { 0.0f, 0.0f, (float)texture.width, (float)texture.height };

    RLDrawBillboardRec(camera, texture, source, position, (Vector2) { scale*fabsf((float)source.width/source.height), scale }, tint);
}

// Draw a billboard (part of a texture defined by a rectangle)
void RLDrawBillboardRec(Camera camera, Texture2D texture, Rectangle source, Vector3 position, Vector2 size, Color tint)
{
    // NOTE: Billboard locked on axis-Y
    Vector3 up = { 0.0f, 1.0f, 0.0f };

    RLDrawBillboardPro(camera, texture, source, position, up, size, RLVector2Scale(size, 0.5), 0.0f, tint);
}

// Draw a billboard with additional parameters
void RLDrawBillboardPro(Camera camera, Texture2D texture, Rectangle source, Vector3 position, Vector3 up, Vector2 size, Vector2 origin, float rotation, Color tint)
{
    // Compute the up vector and the right vector
    Matrix matView = RLMatrixLookAt(camera.position, camera.target, camera.up);
    Vector3 right = { matView.m0, matView.m4, matView.m8 };
    right = RLVector3Scale(right, size.x);
    up = RLVector3Scale(up, size.y);
    ...
    //                right
    Vector3 forward;
    if (rotation != 0.0) forward = RLVector3CrossProduct(right, up);

    Vector3 origin3D = RLVector3Add(RLVector3Scale(RLVector3Normalize(right), origin.x), RLVector3Scale(RLVector3Normalize(up), origin.y));

    Vector3 points[4];
    points[0] = RLVector3Zero();
    points[1] = right;
    points[2] = RLVector3Add(up, right);
    points[3] = up;

    for (int i = 0; i < 4; i++)
    {
        points[i] = RLVector3Subtract(points[i], origin3D);
        if (rotation != 0.0) points[i] = RLVector3RotateByAxisAngle(points[i], forward, rotation * DEG2RAD);
        points[i] = RLVector3Add(points[i], position);
    }
...
void RLDrawBoundingBox(BoundingBox box, Color color)
{
    Vector3 size = { 0 };
    ...
    RLDrawCubeWires(center, size.x, size.y, size.z, color);
}

// Check collision between two spheres
bool RLCheckCollisionSpheres(Vector3 center1, float radius1, Vector3 center2, float radius2)
{
    ...
    */
    ...
    if (RLVector3DotProduct(RLVector3Subtract(center2, center1), RLVector3Subtract(center2, center1)) <= (radius1 + radius2)*(radius1 + radius2)) collision = true;
...
bool RLCheckCollisionBoxes(BoundingBox box1, BoundingBox box2)
{
    bool collision = true;
...
bool RLCheckCollisionBoxSphere(BoundingBox box, Vector3 center, float radius)
{
    bool collision = false;
...
RayCollision RLGetRayCollisionSphere(Ray ray, Vector3 center, float radius)
{
    RayCollision collision = { 0 };

    Vector3 raySpherePos = RLVector3Subtract(center, ray.position);
    float vector = RLVector3DotProduct(raySpherePos, ray.direction);
    float distance = RLVector3Length(raySpherePos);
    ...
    if (distance < radius)
    {
        ...
        collision.point = RLVector3Add(ray.position, RLVector3Scale(ray.direction, collision.distance));

        // Calculate collision normal (pointing outwards)
        collision.normal = RLVector3Negate(RLVector3Normalize(RLVector3Subtract(collision.point, center)));
    }
    else
    {
        collision.distance = vector - sqrtf(d);
        ...
        collision.normal = RLVector3Normalize(RLVector3Subtract(collision.point, center));
    }
...
RayCollision RLGetRayCollisionBox(Ray ray, BoundingBox box)
{
    RayCollision collision = { 0 };
    ...
    if (insideBox) ray.direction = RLVector3Negate(ray.direction);
    ...
    collision.normal = RLVector3Lerp(box.min, box.max, 0.5f);
    // Get vector center point->hit point
    collision.normal = RLVector3Subtract(collision.point, collision.normal);
    // Scale vector to unit cube
    // NOTE: We use an additional .01 to fix numerical errors
    collision.normal = RLVector3Scale(collision.normal, 2.01f);
    collision.normal = RLVector3Divide(collision.normal, RLVector3Subtract(box.max, box.min));
    ...
    collision.normal = RLVector3Normalize(collision.normal);

    if (insideBox)
    {
        // Reset ray.direction
        ray.direction = RLVector3Negate(ray.direction);
        // Fix result
        collision.distance *= -1.0f;
        collision.normal = RLVector3Negate(collision.normal);
    }
...
RayCollision RLGetRayCollisionMesh(Ray ray, Mesh mesh, Matrix transform)
{
    RayCollision collision = { 0 };
    ...
    if (mesh.vertices != NULL)
    {
        ...
        for (int i = 0; i < triangleCount; i++)
        {
            ...
            a = RLVector3Transform(a, transform);
            b = RLVector3Transform(b, transform);
            c = RLVector3Transform(c, transform);

            RayCollision triHitInfo = RLGetRayCollisionTriangle(ray, a, b, c);
...
RayCollision RLGetRayCollisionTriangle(Ray ray, Vector3 p1, Vector3 p2, Vector3 p3)
{
    ...
    edge1 = RLVector3Subtract(p2, p1);
    edge2 = RLVector3Subtract(p3, p1);

    // Begin calculating determinant - also used to calculate u parameter
    p = RLVector3CrossProduct(ray.direction, edge2);

    // If determinant is near zero, ray lies in plane of triangle or ray is parallel to plane of triangle
    det = RLVector3DotProduct(edge1, p);
    ...
    tv = RLVector3Subtract(ray.position, p1);

    // Calculate u parameter and test bound
    u = RLVector3DotProduct(tv, p)*invDet;

    // The intersection lies outside the triangle
    if ((u < 0.0f) || (u > 1.0f)) return collision;

    // Prepare to test v parameter
    q = RLVector3CrossProduct(tv, edge1);

    // Calculate V parameter and test bound
    v = RLVector3DotProduct(ray.direction, q)*invDet;

    // The intersection lies outside the triangle
    if ((v < 0.0f) || ((u + v) > 1.0f)) return collision;

    t = RLVector3DotProduct(edge2, q)*invDet;
    ...
    if (t > EPSILON)
    {
        ...
        collision.normal = RLVector3Normalize(RLVector3CrossProduct(edge1, edge2));
        collision.point = RLVector3Add(ray.position, RLVector3Scale(ray.direction, t));
    }
...
// NOTE: The points are expected to be in counter-clockwise winding
RayCollision RLGetRayCollisionQuad(Ray ray, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4)
{
    RayCollision collision = { 0 };

    collision = RLGetRayCollisionTriangle(ray, p1, p2, p4);

    if (!collision.hit) collision = RLGetRayCollisionTriangle(ray, p2, p3, p4);
...
static void RLBuildPoseFromParentJoints(BoneInfo *bones, int boneCount, Transform *transforms)
{
    for (int i = 0; i < boneCount; i++)
    {
        if (bones[i].parent >= 0)
        {
            ...
            transforms[i].rotation = RLQuaternionMultiply(transforms[bones[i].parent].rotation, transforms[i].rotation);
            transforms[i].translation = RLVector3RotateByQuaternion(transforms[i].translation, transforms[bones[i].parent].rotation);
            transforms[i].translation = RLVector3Add(transforms[i].translation, transforms[bones[i].parent].translation);
            transforms[i].scale = RLVector3Multiply(transforms[i].scale, transforms[bones[i].parent].scale);
        }
    }
}
#endif
...
static Model RLLoadOBJ(const char *fileName)
{
    ...
    strcpy(currentDir, RLGetWorkingDirectory()); // Save current working directory
    const char* workingDir = RLGetDirectoryPath(fileName); // Switch to OBJ directory for material path correctness
    ...
    if (objMaterialCount > 0) RLProcessMaterialsOBJ(model.materials, objMaterials, objMaterialCount);
    else model.materials[0] = RLLoadMaterialDefault(); // Set default material for the mesh
    ...
    for (int i = 0; i < model.meshCount; i++)
        RLUploadMesh(model.meshes + i, true);
...
#if defined(SUPPORT_FILEFORMAT_IQM)
// Load IQM mesh data
static Model RLLoadIQM(const char *fileName)
{
    ...
    */
    ...
    const char *basePath = RLGetDirectoryPath(fileName);
    ...
    for (int i = 0; i < model.meshCount; i++)
    {
        ...
        model.materials[i].maps[MATERIAL_MAP_ALBEDO].texture = RLLoadTexture(RLTextFormat("%s/%s", basePath, material));
    ...
    RLBuildPoseFromParentJoints(model.bones, model.boneCount, model.bindPose);
    ...
    for (int i = 0; i < model.meshCount; i++)
    {
        ...
        {
            model.meshes[i].boneMatrices[j] = RLMatrixIdentity();
        }
    }
...
static ModelAnimation * RLLoadModelAnimationsIQM(const char *fileName, int *animCount)
{
    #define IQM_MAGIC       "INTERQUAKEMODEL"   // IQM file magic number
    #define IQM_VERSION     2                   // only IQM version 2 supported
    ...
    *animCount = iqmHeader->num_anims;
    ...
    for (unsigned int a = 0; a < iqmHeader->num_anims; a++)
    {
        ...
        RLTraceLog(LOG_INFO, "IQM Anim %s", animations[a].name);
        ...
        for (unsigned int frame = 0; frame < anim[a].num_frames; frame++)
        {
            for (unsigned int i = 0; i < iqmHeader->num_poses; i++)
            {
                ...
                animations[a].framePoses[frame][i].rotation = RLQuaternionNormalize(animations[a].framePoses[frame][i].rotation);
            }
        }
        ...
        for (unsigned int frame = 0; frame < anim[a].num_frames; frame++)
        {
            for (int i = 0; i < animations[a].boneCount; i++)
            {
                ...
                {
                    animations[a].framePoses[frame][i].rotation = RLQuaternionMultiply(animations[a].framePoses[frame][animations[a].bones[i].parent].rotation, animations[a].framePoses[frame][i].rotation);
                    animations[a].framePoses[frame][i].translation = RLVector3RotateByQuaternion(animations[a].framePoses[frame][i].translation, animations[a].framePoses[frame][animations[a].bones[i].parent].rotation);
                    animations[a].framePoses[frame][i].translation = RLVector3Add(animations[a].framePoses[frame][i].translation, animations[a].framePoses[frame][animations[a].bones[i].parent].translation);
                    animations[a].framePoses[frame][i].scale = RLVector3Multiply(animations[a].framePoses[frame][i].scale, animations[a].framePoses[frame][animations[a].bones[i].parent].scale);
                }
            }
        }
    }
...
static cgltf_result RLLoadFileGLTFCallback(const struct cgltf_memory_options *memoryOptions, const struct cgltf_file_options *fileOptions, const char *path, cgltf_size *size, void **data)
{
    int filesize;
    unsigned char *filedata = RLLoadFileData(path, &filesize);
...
static void RLReleaseFileGLTFCallback(const struct cgltf_memory_options *memoryOptions, const struct cgltf_file_options *fileOptions, void *data)
{
    RLUnloadFileData(data);
}

// Load image from different glTF provided methods (uri, path, buffer_view)
static Image RLLoadImageFromCgltfImage(cgltf_image *cgltfImage, const char *texPath)
{
    Image image = { 0 };
    ...
    if (cgltfImage->uri != NULL)     // Check if image data is provided as an uri (base64 or path)
    {
            ...
            (cgltfImage->uri[4] == ':'))     // Check if image is provided as base64 text data
        {
            ...
            else
            {
                ...
                options.file.read = RLLoadFileGLTFCallback;
                options.file.release = RLReleaseFileGLTFCallback;
                cgltf_result result = cgltf_load_buffer_base64(&options, outSize, cgltfImage->uri + i + 1, &data);

                if (result == cgltf_result_success)
                {
                    image = RLLoadImageFromMemory(".png", (unsigned char *)data, outSize);
                    RL_FREE(data);
                }
            }
        }
    ...
    else if (cgltfImage->buffer_view->buffer->data != NULL)    // Check if image is provided as data buffer
    {
        ...
        if ((strcmp(cgltfImage->mime_type, "image\\/png") == 0) ||
            (strcmp(cgltfImage->mime_type, "image/png") == 0)) image = RLLoadImageFromMemory(".png", data, (int)cgltfImage->buffer_view->size);
        else if ((strcmp(cgltfImage->mime_type, "image\\/jpeg") == 0) ||
                 (strcmp(cgltfImage->mime_type, "image/jpeg") == 0)) image = RLLoadImageFromMemory(".jpg", data, (int)cgltfImage->buffer_view->size);
        else TRACELOG(LOG_WARNING, "MODEL: glTF image data MIME type not recognized", RLTextFormat("%s/%s", texPath, cgltfImage->uri));
...
static BoneInfo * RLLoadBoneInfoGLTF(cgltf_skin skin, int *boneCount)
{
...
static Model RLLoadGLTF(const char *fileName)
{
    /*********************************************************************************************
    ...
    ***********************************************************************************************/
    ...
    if (result == cgltf_result_success)
    {
        ...
        model.materials[0] = RLLoadMaterialDefault();     // Load default material (index: 0)
        ...
        {
            model.materials[j] = RLLoadMaterialDefault();
            const char *texPath = RLGetDirectoryPath(fileName);
            ...
            if (data->materials[i].has_pbr_metallic_roughness)
            {
                ...
                {
                    Image imAlbedo = RLLoadImageFromCgltfImage(data->materials[i].pbr_metallic_roughness.base_color_texture.texture->image, texPath);
                    if (imAlbedo.data != NULL)
                    {
                        model.materials[j].maps[MATERIAL_MAP_ALBEDO].texture = RLLoadTextureFromImage(imAlbedo);
                        RLUnloadImage(imAlbedo);
                    }
                }
                ...
                {
                    Image imMetallicRoughness = RLLoadImageFromCgltfImage(data->materials[i].pbr_metallic_roughness.metallic_roughness_texture.texture->image, texPath);
                    if (imMetallicRoughness.data != NULL)
                    {
                        model.materials[j].maps[MATERIAL_MAP_ROUGHNESS].texture = RLLoadTextureFromImage(imMetallicRoughness);
                        RLUnloadImage(imMetallicRoughness);
                    }
                ...
                {
                    Image imNormal = RLLoadImageFromCgltfImage(data->materials[i].normal_texture.texture->image, texPath);
                    if (imNormal.data != NULL)
                    {
                        model.materials[j].maps[MATERIAL_MAP_NORMAL].texture = RLLoadTextureFromImage(imNormal);
                        RLUnloadImage(imNormal);
                    }
                }
                ...
                {
                    Image imOcclusion = RLLoadImageFromCgltfImage(data->materials[i].occlusion_texture.texture->image, texPath);
                    if (imOcclusion.data != NULL)
                    {
                        model.materials[j].maps[MATERIAL_MAP_OCCLUSION].texture = RLLoadTextureFromImage(imOcclusion);
                        RLUnloadImage(imOcclusion);
                    }
                }
                ...
                {
                    Image imEmissive = RLLoadImageFromCgltfImage(data->materials[i].emissive_texture.texture->image, texPath);
                    if (imEmissive.data != NULL)
                    {
                        model.materials[j].maps[MATERIAL_MAP_EMISSION].texture = RLLoadTextureFromImage(imEmissive);
                        RLUnloadImage(imEmissive);
                    }
        ...
        for (unsigned int i = 0; i < data->nodes_count; i++)
        {
            ...
            Matrix worldMatrixNormals = RLMatrixTranspose(RLMatrixInvert(worldMatrix));
                ...
                for (unsigned int j = 0; j < mesh->primitives[p].attributes_count; j++)
                {
                    ...
                    if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_position)      // POSITION, vec3, float
                    {
                        ...
                        if ((attribute->type == cgltf_type_vec3) && (attribute->component_type == cgltf_component_type_r_32f))
                        {
                            ...
                            {
                                Vector3 vt = RLVector3Transform((Vector3){ vertices[3*k], vertices[3*k+1], vertices[3*k+2] }, worldMatrix);
                                vertices[3*k] = vt.x;
                                vertices[3*k+1] = vt.y;
                                vertices[3*k+2] = vt.z;
                            }
                    ...
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_normal)   // NORMAL, vec3, float
                    {
                        ...
                        if ((attribute->type == cgltf_type_vec3) && (attribute->component_type == cgltf_component_type_r_32f))
                        {
                            ...
                            for (unsigned int k = 0; k < attribute->count; k++)
                            {
                                Vector3 nt = RLVector3Transform((Vector3){ normals[3*k], normals[3*k+1], normals[3*k+2] }, worldMatrixNormals);
                                normals[3*k] = nt.x;
                                normals[3*k+1] = nt.y;
                                normals[3*k+2] = nt.z;
                            }
                    ...
                    else if (mesh->primitives[p].attributes[j].type == cgltf_attribute_type_tangent)   // TANGENT, vec3, float
                    {
                        ...
                        if ((attribute->type == cgltf_type_vec4) && (attribute->component_type == cgltf_component_type_r_32f))
                        {
                            ...
                            for (unsigned int k = 0; k < attribute->count; k++)
                            {
                                Vector3 tt = RLVector3Transform((Vector3){ tangents[3*k], tangents[3*k+1], tangents[3*k+2] }, worldMatrix);
                                tangents[3*k] = tt.x;
                                tangents[3*k+1] = tt.y;
                                tangents[3*k+2] = tt.z;
                            }
        ...
        if (data->skins_count > 0)
        {
            ...
            model.bones = RLLoadBoneInfoGLTF(skin, &model.boneCount);
            ...
            for (int i = 0; i < model.boneCount; i++)
            {
                ...
                RLMatrixDecompose(worldMatrix, &(model.bindPose[i].translation), &(model.bindPose[i].rotation), &(model.bindPose[i].scale));
            }
        }
        ...
        for (unsigned int i = 0; i < data->nodes_count; i++)
        {
            ...
            for (unsigned int p = 0; p < mesh->primitives_count; p++)
            {
                ...
                {
                    model.meshes[meshIndex].boneMatrices[j] = RLMatrixIdentity();
                }
...
static bool RLGetPoseAtTimeGLTF(cgltf_interpolation_type interpolationType, cgltf_accessor *input, cgltf_accessor *output, float time, void *data)
{
    ...
    if (RLFloatEquals(tend, tstart)) return true;
    ...
    if (output->type == cgltf_type_vec3)
    {
        switch (interpolationType)
        {
            ...
            case cgltf_interpolation_type_cubic_spline:
            {
                ...
                *r = RLVector3CubicHermite(v1, tangent1, v2, tangent2, t);
            } break;
            default: break;
        }
    }
    else if (output->type == cgltf_type_vec4)
    {
        ...
        switch (interpolationType)
        {
            ...
            case cgltf_interpolation_type_cubic_spline:
            {
                ...
                v1 = RLQuaternionNormalize(v1);
                v2 = RLQuaternionNormalize(v2);

                if (RLVector4DotProduct(v1, v2) < 0.0f)
                {
                    v2 = RLVector4Negate(v2);
                }

                outTangent1 = RLVector4Scale(outTangent1, duration);
                inTangent2 = RLVector4Scale(inTangent2, duration);

                *r = RLQuaternionCubicHermiteSpline(v1, outTangent1, v2, inTangent2, t);
            } break;
            default: break;
        }
    }
...
static ModelAnimation * RLLoadModelAnimationsGLTF(const char *fileName, int *animCount)
{
    // glTF file loading
    int dataSize = 0;
    unsigned char *fileData = RLLoadFileData(fileName, &dataSize);
    ...
    if (result == cgltf_result_success)
    {
        if (data->skins_count > 0)
        {
            ...
            *animCount = (int)data->animations_count;
            ...
            {
                animations[i].bones = RLLoadBoneInfoGLTF(skin, &animations[i].boneCount);
                ...
                for (int j = 0; j < animations[i].frameCount; j++)
                {
                    ...
                    for (int k = 0; k < animations[i].boneCount; k++)
                    {
                        ...
                        {
                            if (!RLGetPoseAtTimeGLTF(boneChannels[k].interpolationType, boneChannels[k].translate->sampler->input, boneChannels[k].translate->sampler->output, time, &translation))
                            {
                        ...
                        {
                            if (!RLGetPoseAtTimeGLTF(boneChannels[k].interpolationType, boneChannels[k].rotate->sampler->input, boneChannels[k].rotate->sampler->output, time, &rotation))
                            {
                        ...
                        {
                            if (!RLGetPoseAtTimeGLTF(boneChannels[k].interpolationType, boneChannels[k].scale->sampler->input, boneChannels[k].scale->sampler->output, time, &scale))
                            {
                    ...
                    RLBuildPoseFromParentJoints(animations[i].bones, animations[i].boneCount, animations[i].framePoses[j]);
                }
...
static Model RLLoadVOX(const char *fileName)
{
    Model model = { 0 };
    ...
    int ret = RLVox_LoadFromMemory(fileData, dataSize, &voxarray);
...
unsigned char *m3d_loaderhook(char *fn, unsigned int *len) { return RLLoadFileData((const char *)fn, (int *)len); }
void m3d_freehook(void *data) { RLUnloadFileData((unsigned char *)data); }

// Load M3D mesh data
static Model RLLoadM3D(const char *fileName)
{
    Model model = { 0 };
    ...
    if (fileData != NULL)
    {
        ...
        */
        ...
        {
            model.materials[i + 1] = RLLoadMaterialDefault();
                ...
                switch (prop->type)
                {
                    ...
                    default:
                    {
                        if (prop->type >= 128)
                        {
                            ...
                            switch (prop->type)
                            {
                                case m3dp_map_Kd: model.materials[i + 1].maps[MATERIAL_MAP_DIFFUSE].texture = RLLoadTextureFromImage(image); break;
                                case m3dp_map_Ks: model.materials[i + 1].maps[MATERIAL_MAP_SPECULAR].texture = RLLoadTextureFromImage(image); break;
                                case m3dp_map_Ke: model.materials[i + 1].maps[MATERIAL_MAP_EMISSION].texture = RLLoadTextureFromImage(image); break;
                                case m3dp_map_Km: model.materials[i + 1].maps[MATERIAL_MAP_NORMAL].texture = RLLoadTextureFromImage(image); break;
                                case m3dp_map_Ka: model.materials[i + 1].maps[MATERIAL_MAP_OCCLUSION].texture = RLLoadTextureFromImage(image); break;
                                case m3dp_map_Pm: model.materials[i + 1].maps[MATERIAL_MAP_ROUGHNESS].texture = RLLoadTextureFromImage(image); break;
                                default: break;
                            }
                        }
                    } break;
        ...
        if (m3d->numbone)
        {
            ...
            for (i = 0; i < (int)m3d->numbone; i++)
            {
                ...
                model.bindPose[i].rotation = RLQuaternionNormalize(model.bindPose[i].rotation);
                ...
                {
                    model.bindPose[i].rotation = RLQuaternionMultiply(model.bindPose[model.bones[i].parent].rotation, model.bindPose[i].rotation);
                    model.bindPose[i].translation = RLVector3RotateByQuaternion(model.bindPose[i].translation, model.bindPose[model.bones[i].parent].rotation);
                    model.bindPose[i].translation = RLVector3Add(model.bindPose[i].translation, model.bindPose[model.bones[i].parent].translation);
                    model.bindPose[i].scale = RLVector3Multiply(model.bindPose[i].scale, model.bindPose[model.bones[i].parent].scale);
                }
            }
...
static ModelAnimation * RLLoadModelAnimationsM3D(const char *fileName, int *animCount)
{
    ModelAnimation *animations = NULL;
            ...
            for (i = 0; i < animations[a].frameCount; i++)
            {
                ...
                if (pose != NULL)
                {
                    for (j = 0; j < (int)m3d->numbone; j++)
                    {
                        ...
                        animations[a].framePoses[i][j].rotation = RLQuaternionNormalize(animations[a].framePoses[i][j].rotation);
                        ...
                        {
                            animations[a].framePoses[i][j].rotation = RLQuaternionMultiply(animations[a].framePoses[i][animations[a].bones[j].parent].rotation, animations[a].framePoses[i][j].rotation);
                            animations[a].framePoses[i][j].translation = RLVector3RotateByQuaternion(animations[a].framePoses[i][j].translation, animations[a].framePoses[i][animations[a].bones[j].parent].rotation);
                            animations[a].framePoses[i][j].translation = RLVector3Add(animations[a].framePoses[i][j].translation, animations[a].framePoses[i][animations[a].bones[j].parent].translation);
                            animations[a].framePoses[i][j].scale = RLVector3Multiply(animations[a].framePoses[i][j].scale, animations[a].framePoses[i][animations[a].bones[j].parent].scale);
                        }
                    }
