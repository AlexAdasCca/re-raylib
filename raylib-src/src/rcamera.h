...
Vector3 RLGetCameraRight(Camera *camera)
{
    ...
    return RLVector3Normalize(RLVector3CrossProduct(forward, up));
}
...
void RLCameraYaw(Camera *camera, float angle, bool rotateAroundTarget)
{
    ...
    // View vector
    Vector3 targetPosition = RLVector3Subtract(camera->target, camera->position);
...
Matrix RLGetCameraProjectionMatrix(Camera *camera, float aspect)
{
    ...
    {
        return RLMatrixPerspective(camera->fovy*DEG2RAD, aspect, CAMERA_CULL_DISTANCE_NEAR, CAMERA_CULL_DISTANCE_FAR);
    }
    else if (camera->projection == CAMERA_ORTHOGRAPHIC)
    {
        ...
        return RLMatrixOrtho(-right, right, -top, top, CAMERA_CULL_DISTANCE_NEAR, CAMERA_CULL_DISTANCE_FAR);
    }
...
void RLUpdateCamera(Camera *camera, int mode)
{
    Vector2 mousePositionDelta = RLGetMouseDelta();
    ...
    float cameraMoveSpeed = CAMERA_MOVE_SPEED* RLGetFrameTime();
    float cameraRotationSpeed = CAMERA_ROTATION_SPEED* RLGetFrameTime();
    float cameraPanSpeed = CAMERA_PAN_SPEED* RLGetFrameTime();
    float cameraOrbitalSpeed = CAMERA_ORBITAL_SPEED* RLGetFrameTime();

    if (mode == CAMERA_CUSTOM) {}
    else if (mode == CAMERA_ORBITAL)
    {
        // Orbital can just orbit
        Matrix rotation = RLMatrixRotate(RLGetCameraUp(camera), cameraOrbitalSpeed);
        Vector3 view = RLVector3Subtract(camera->position, camera->target);
    ...
    {
        // Camera rotation
        if (RLIsKeyDown(KEY_DOWN)) RLCameraPitch(camera, -cameraRotationSpeed, lockView, rotateAroundTarget, rotateUp);
        if (RLIsKeyDown(KEY_UP)) RLCameraPitch(camera, cameraRotationSpeed, lockView, rotateAroundTarget, rotateUp);
        if (RLIsKeyDown(KEY_RIGHT)) RLCameraYaw(camera, -cameraRotationSpeed, rotateAroundTarget);
        if (RLIsKeyDown(KEY_LEFT)) RLCameraYaw(camera, cameraRotationSpeed, rotateAroundTarget);
        if (RLIsKeyDown(KEY_Q)) RLCameraRoll(camera, -cameraRotationSpeed);
        if (RLIsKeyDown(KEY_E)) RLCameraRoll(camera, cameraRotationSpeed);

        // Camera movement
        // Camera pan (for CAMERA_FREE)
        if ((mode == CAMERA_FREE) && (RLIsMouseButtonDown(MOUSE_BUTTON_MIDDLE)))
        {
            const Vector2 mouseDelta = RLGetMouseDelta();
            if (mouseDelta.x > 0.0f) RLCameraMoveRight(camera, cameraPanSpeed, moveInWorldPlane);
            if (mouseDelta.x < 0.0f) RLCameraMoveRight(camera, -cameraPanSpeed, moveInWorldPlane);
            if (mouseDelta.y > 0.0f) RLCameraMoveUp(camera, -cameraPanSpeed);
            if (mouseDelta.y < 0.0f) RLCameraMoveUp(camera, cameraPanSpeed);
        }
        else
        {
            // Mouse support
            RLCameraYaw(camera, -mousePositionDelta.x*CAMERA_MOUSE_MOVE_SENSITIVITY, rotateAroundTarget);
            RLCameraPitch(camera, -mousePositionDelta.y*CAMERA_MOUSE_MOVE_SENSITIVITY, lockView, rotateAroundTarget, rotateUp);
        }

        // Keyboard support
        if (RLIsKeyDown(KEY_W)) RLCameraMoveForward(camera, cameraMoveSpeed, moveInWorldPlane);
        if (RLIsKeyDown(KEY_A)) RLCameraMoveRight(camera, -cameraMoveSpeed, moveInWorldPlane);
        if (RLIsKeyDown(KEY_S)) RLCameraMoveForward(camera, -cameraMoveSpeed, moveInWorldPlane);
        if (RLIsKeyDown(KEY_D)) RLCameraMoveRight(camera, cameraMoveSpeed, moveInWorldPlane);

        // Gamepad movement
        if (RLIsGamepadAvailable(0))
        {
            // Gamepad controller support
            RLCameraYaw(camera, -(RLGetGamepadAxisMovement(0, GAMEPAD_AXIS_RIGHT_X)*2)*CAMERA_MOUSE_MOVE_SENSITIVITY, rotateAroundTarget);
            RLCameraPitch(camera, -(RLGetGamepadAxisMovement(0, GAMEPAD_AXIS_RIGHT_Y)*2)*CAMERA_MOUSE_MOVE_SENSITIVITY, lockView, rotateAroundTarget, rotateUp);

            if (RLGetGamepadAxisMovement(0, GAMEPAD_AXIS_LEFT_Y) <= -0.25f) RLCameraMoveForward(camera, cameraMoveSpeed, moveInWorldPlane);
            if (RLGetGamepadAxisMovement(0, GAMEPAD_AXIS_LEFT_X) <= -0.25f) RLCameraMoveRight(camera, -cameraMoveSpeed, moveInWorldPlane);
            if (RLGetGamepadAxisMovement(0, GAMEPAD_AXIS_LEFT_Y) >= 0.25f) RLCameraMoveForward(camera, -cameraMoveSpeed, moveInWorldPlane);
            if (RLGetGamepadAxisMovement(0, GAMEPAD_AXIS_LEFT_X) >= 0.25f) RLCameraMoveRight(camera, cameraMoveSpeed, moveInWorldPlane);
        }
        ...
        if (mode == CAMERA_FREE)
        {
            if (RLIsKeyDown(KEY_SPACE)) RLCameraMoveUp(camera, cameraMoveSpeed);
            if (RLIsKeyDown(KEY_LEFT_CONTROL)) RLCameraMoveUp(camera, -cameraMoveSpeed);
        }
    }
    ...
    if ((mode == CAMERA_THIRD_PERSON) || (mode == CAMERA_ORBITAL) || (mode == CAMERA_FREE))
    {
        ...
        if (RLIsKeyPressed(KEY_KP_SUBTRACT)) RLCameraMoveToTarget(camera, 2.0f);
        if (RLIsKeyPressed(KEY_KP_ADD)) RLCameraMoveToTarget(camera, -2.0f);
    }
}
#endif // !RCAMERA_STANDALONE

// Update camera movement, movement/rotation values should be provided by user
void RLUpdateCameraPro(Camera *camera, Vector3 movement, Vector3 rotation, float zoom)
{
    // Required values
    ...
    // Camera rotation
    RLCameraPitch(camera, -rotation.y*DEG2RAD, lockView, rotateAroundTarget, rotateUp);
    RLCameraYaw(camera, -rotation.x*DEG2RAD, rotateAroundTarget);
    RLCameraRoll(camera, rotation.z*DEG2RAD);
