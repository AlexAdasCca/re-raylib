...
//----------------------------------------------------------------------------------
int RLInitPlatform(void);          // Initialize platform (graphics, inputs and more)
void RLClosePlatform(void);        // Close platform

// Error callback event
static void RLErrorCallback(int error, const char *description);                             // GLFW3 Error Callback, runs on GLFW3 error

// Window callbacks events
static void RLWindowSizeCallback(GLFWwindow *window, int width, int height);                 // GLFW3 WindowSize Callback, runs when window is resized
static void RLWindowPosCallback(GLFWwindow* window, int x, int y);                     // GLFW3 WindowPos Callback, runs when window is moved
static void RLWindowIconifyCallback(GLFWwindow *window, int iconified);                      // GLFW3 WindowIconify Callback, runs when window is minimized/restored
static void RLWindowMaximizeCallback(GLFWwindow* window, int maximized);                     // GLFW3 Window Maximize Callback, runs when window is maximized
static void RLWindowFocusCallback(GLFWwindow *window, int focused);                          // GLFW3 WindowFocus Callback, runs when window get/lose focus
static void RLWindowDropCallback(GLFWwindow *window, int count, const char **paths);         // GLFW3 Window Drop Callback, runs when drop files into window
static void RLWindowContentScaleCallback(GLFWwindow *window, float scalex, float scaley);    // GLFW3 Window Content Scale Callback, runs when window changes scale

// Input callbacks events
static void RLKeyCallback(GLFWwindow *window, int key, int scancode, int action, int mods);  // GLFW3 Keyboard Callback, runs on key pressed
static void RLCharCallback(GLFWwindow *window, unsigned int codepoint);                      // GLFW3 Char Callback, runs on key pressed (get codepoint value)
static void RLMouseButtonCallback(GLFWwindow *window, int button, int action, int mods);     // GLFW3 Mouse Button Callback, runs on mouse button pressed
static void RLMouseCursorPosCallback(GLFWwindow *window, double x, double y);                // GLFW3 Cursor Position Callback, runs on mouse move
static void RLMouseScrollCallback(GLFWwindow *window, double xoffset, double yoffset);       // GLFW3 Scrolling Callback, runs on mouse wheel
static void RLCursorEnterCallback(GLFWwindow *window, int enter);                            // GLFW3 Cursor Enter Callback, cursor enters client area
static void RLJoystickCallback(int jid, int event);                                           // GLFW3 Joystick Connected/Disconnected Callback
...
void RLToggleBorderlessWindowed(void)
{
    ...
    if ((monitor >= 0) && (monitor < monitorCount))
    {
        ...
        if (mode)
        {
            if (!RLIsWindowState(FLAG_BORDERLESS_WINDOWED_MODE))
            {
...
// Get clipboard image
Image RLGetClipboardImage(void)
{
    Image image = {0};
    ...
    {
        image = RLLoadImageFromMemory(".bmp", fileData, (int)dataSize);
    }
    return image;
}
...
void RLPollInputEvents(void)
{
    ...
    while (RLIsWindowState(FLAG_WINDOW_MINIMIZED) && !RLIsWindowState(FLAG_WINDOW_ALWAYS_RUN)) glfwWaitEvents();
...
static void RLSetDimensionsFromMonitor(GLFWmonitor *monitor)
{
  const GLFWvidmode *mode = glfwGetVideoMode(monitor);
...
int RLInitPlatform(void)
{
    glfwSetErrorCallback(RLErrorCallback);
/*
...
*/
    ...
    if (CORE.Window.fullscreen)
    {
        ...
        RLSetDimensionsFromMonitor(monitor);
        ...
        RLSetupFramebuffer(CORE.Window.display.width, CORE.Window.display.height);
    ...
    if ((CORE.Window.flags & FLAG_WINDOW_MINIMIZED) > 0) RLMinimizeWindow();
    ...
    glfwSetWindowSizeCallback(platform.handle, RLWindowSizeCallback);      // NOTE: Resizing not allowed by default!
    glfwSetWindowPosCallback(platform.handle, RLWindowPosCallback);
    glfwSetWindowMaximizeCallback(platform.handle, RLWindowMaximizeCallback);
    glfwSetWindowIconifyCallback(platform.handle, RLWindowIconifyCallback);
    glfwSetWindowFocusCallback(platform.handle, RLWindowFocusCallback);
    glfwSetDropCallback(platform.handle, RLWindowDropCallback);

    if ((CORE.Window.flags & FLAG_WINDOW_HIGHDPI) > 0)
    {
       glfwSetWindowContentScaleCallback(platform.handle, RLWindowContentScaleCallback);
    }

    // Set input callback events
    glfwSetKeyCallback(platform.handle, RLKeyCallback);
    glfwSetCharCallback(platform.handle, RLCharCallback);
    glfwSetMouseButtonCallback(platform.handle, RLMouseButtonCallback);
    glfwSetCursorPosCallback(platform.handle, RLMouseCursorPosCallback);   // Track mouse position changes
    glfwSetScrollCallback(platform.handle, RLMouseScrollCallback);
    glfwSetCursorEnterCallback(platform.handle, RLCursorEnterCallback);
    glfwSetJoystickCallback(RLJoystickCallback);
...
static void RLErrorCallback(int error, const char *description)
{
...
static void RLWindowSizeCallback(GLFWwindow *window, int width, int height)
{
    // Reset viewport and projection matrix for new size
    RLSetupViewport(width, height);
...
static void RLWindowPosCallback(GLFWwindow* window, int x, int y)
{
    // Set current window position
    CORE.Window.position.x = x;
    CORE.Window.position.y = y;
}
static void RLWindowContentScaleCallback(GLFWwindow *window, float scalex, float scaley)
{
    CORE.Window.screenScale = RLMatrixScale(scalex, scaley, 1.0f);
}

// GLFW3 WindowIconify Callback, runs when window is minimized/restored
static void RLWindowIconifyCallback(GLFWwindow *window, int iconified)
{
...
static void RLWindowMaximizeCallback(GLFWwindow *window, int maximized)
{
...
static void RLWindowFocusCallback(GLFWwindow *window, int focused)
{
...
static void RLWindowDropCallback(GLFWwindow *window, int count, const char **paths)
{
    if (count > 0)
    {
...
static void RLKeyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
...
static void RLCharCallback(GLFWwindow *window, unsigned int codepoint)
{
...
static void RLMouseButtonCallback(GLFWwindow *window, int button, int action, int mods)
{
...
static void RLMouseCursorPosCallback(GLFWwindow *window, double x, double y)
{
...
static void RLMouseScrollCallback(GLFWwindow *window, double xoffset, double yoffset)
{
    CORE.Input.Mouse.currentWheelMove = (Vector2){ (float)xoffset, (float)yoffset };
}

// GLFW3 CursorEnter Callback, when cursor enters the window
static void RLCursorEnterCallback(GLFWwindow *window, int enter)
{
...
static void RLJoystickCallback(int jid, int event)
{
