...
//----------------------------------------------------------------------------------
static void RLOnLog(void *pUserData, ma_uint32 level, const char *pMessage);

// Reads audio data from an AudioBuffer object in internal/device formats
static ma_uint32 RLReadAudioBufferFramesInInternalFormat(AudioBuffer *audioBuffer, void *framesOut, ma_uint32 frameCount);
static ma_uint32 RLReadAudioBufferFramesInMixingFormat(AudioBuffer *audioBuffer, float *framesOut, ma_uint32 frameCount);

static void RLOnSendAudioDataToDevice(ma_device *pDevice, void *pFramesOut, const void *pFramesInput, ma_uint32 frameCount);
static void RLMixAudioFrames(float *framesOut, const float *framesIn, ma_uint32 frameCount, AudioBuffer *buffer);

static bool RLIsAudioBufferPlayingInLockedState(AudioBuffer *buffer);
static void RLStopAudioBufferInLockedState(AudioBuffer *buffer);
static void RLUpdateAudioStreamInLockedState(AudioStream stream, const void *data, int frameCount);
...
//----------------------------------------------------------------------------------
AudioBuffer * RLLoadAudioBuffer(ma_format format, ma_uint32 channels, ma_uint32 sampleRate, ma_uint32 sizeInFrames, int usage);
void RLUnloadAudioBuffer(AudioBuffer *buffer);

bool RLIsAudioBufferPlaying(AudioBuffer *buffer);
void RLPlayAudioBuffer(AudioBuffer *buffer);
void RLStopAudioBuffer(AudioBuffer *buffer);
void RLPauseAudioBuffer(AudioBuffer *buffer);
void RLResumeAudioBuffer(AudioBuffer *buffer);
void RLSetAudioBufferVolume(AudioBuffer *buffer, float volume);
void RLSetAudioBufferPitch(AudioBuffer *buffer, float pitch);
void RLSetAudioBufferPan(AudioBuffer *buffer, float pan);
void RLTrackAudioBuffer(AudioBuffer *buffer);
void RLUntrackAudioBuffer(AudioBuffer *buffer);
...
void RLInitAudioDevice(void)
{
    ...
    ma_log_callback_init(RLOnLog, NULL);
    ...
    config.dataCallback = RLOnSendAudioDataToDevice;
...
AudioBuffer * RLLoadAudioBuffer(ma_format format, ma_uint32 channels, ma_uint32 sampleRate, ma_uint32 sizeInFrames, int usage)
{
    ...
    RLTrackAudioBuffer(audioBuffer);
...
bool RLIsAudioBufferPlaying(AudioBuffer *buffer)
{
    ...
    result = RLIsAudioBufferPlayingInLockedState(buffer);
...
{
    ma_mutex_lock(&AUDIO.System.lock);
    RLStopAudioBufferInLockedState(buffer);
    ma_mutex_unlock(&AUDIO.System.lock);
}
...
Wave RLLoadWave(const char *fileName)
{
    Wave wave = { 0 };
...
Wave RLLoadWaveFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    Wave wave = { 0 };
...
Sound RLLoadSound(const char *fileName)
{
    Wave wave = RLLoadWave(fileName);
...
Sound RLLoadSoundFromWave(Wave wave)
{
    Sound sound = { 0 };
    ...
    if (wave.data != NULL)
    {
        ...
        AudioBuffer *audioBuffer = RLLoadAudioBuffer(AUDIO_DEVICE_FORMAT, AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate, frameCount, AUDIO_BUFFER_USAGE_STATIC);
        if (audioBuffer == NULL)
        {
...
Sound RLLoadSoundAlias(Sound source)
{
    Sound sound = { 0 };

    if (source.stream.buffer->data != NULL)
    {
        AudioBuffer *audioBuffer = RLLoadAudioBuffer(AUDIO_DEVICE_FORMAT, AUDIO_DEVICE_CHANNELS, AUDIO.System.device.sampleRate, 0, AUDIO_BUFFER_USAGE_STATIC);
...
bool RLIsSoundValid(Sound sound)
{
    bool result = false;
...
// Unload sound
void RLUnloadSound(Sound sound)
{
    RLUnloadAudioBuffer(sound.stream.buffer);
    //TRACELOG(LOG_INFO, "SOUND: Unloaded sound data from RAM");
}

void RLUnloadSoundAlias(Sound alias)
{
    ...
    {
        RLUntrackAudioBuffer(alias.stream.buffer);
...
void RLUpdateSound(Sound sound, const void *data, int frameCount)
{
    if (sound.stream.buffer != NULL)
    {
        RLStopAudioBuffer(sound.stream.buffer);
...
bool RLExportWave(Wave wave, const char *fileName)
{
    bool success = false;
...
bool RLExportWaveAsCode(Wave wave, const char *fileName)
{
    bool success = false;
...
{
    RLPlayAudioBuffer(sound.stream.buffer);
}

// Pause a sound
void RLPauseSound(Sound sound)
{
    RLPauseAudioBuffer(sound.stream.buffer);
}

// Resume a paused sound
void RLResumeSound(Sound sound)
{
    RLResumeAudioBuffer(sound.stream.buffer);
}
...
bool RLIsSoundPlaying(Sound sound)
{
    bool result = false;

    if (RLIsAudioBufferPlaying(sound.stream.buffer)) result = true;

    return result;
}

// Set volume for a sound
void RLSetSoundVolume(Sound sound, float volume)
{
    RLSetAudioBufferVolume(sound.stream.buffer, volume);
}

// Set pitch for a sound
void RLSetSoundPitch(Sound sound, float pitch)
{
    RLSetAudioBufferPitch(sound.stream.buffer, pitch);
}

// Set pan for a sound
void RLSetSoundPan(Sound sound, float pan)
{
    RLSetAudioBufferPan(sound.stream.buffer, pan);
}

// Convert wave data to desired format
void RLWaveFormat(Wave *wave, int sampleRate, int sampleSize, int channels)
{
    ma_format formatIn = ((wave->sampleSize == 8)? ma_format_u8 : ((wave->sampleSize == 16)? ma_format_s16 : ma_format_f32));
...
void RLWaveCrop(Wave *wave, int initFrame, int finalFrame)
{
...
float * RLLoadWaveSamples(Wave wave)
{
...
Music RLLoadMusicStream(const char *fileName)
{
    Music music = { 0 };
...
// WARNING: File extension must be provided in lower-case
Music RLLoadMusicStreamFromMemory(const char *fileType, const unsigned char *data, int dataSize)
{
    Music music = { 0 };
    bool musicLoaded = false;
...
bool RLIsMusicValid(Music music)
{
...
// Unload music stream
void RLUnloadMusicStream(Music music)
{
...
void RLPlayMusicStream(Music music)
{
    RLPlayAudioStream(music.stream);
}

// Pause music playing
void RLPauseMusicStream(Music music)
{
    RLPauseAudioStream(music.stream);
}

// Resume music playing
void RLResumeMusicStream(Music music)
{
    RLResumeAudioStream(music.stream);
}

// Stop music playing (close stream)
void RLStopMusicStream(Music music)
{
...
void RLSeekMusicStream(Music music, float position)
{
...
void RLUpdateMusicStream(Music music)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        RLUpdateAudioStreamInLockedState(music.stream, AUDIO.System.pcmBuffer, framesToStream);
...
bool RLIsMusicStreamPlaying(Music music)
{
    return RLIsAudioStreamPlaying(music.stream);
}

// Set volume for music
void RLSetMusicVolume(Music music, float volume)
{
    RLSetAudioStreamVolume(music.stream, volume);
}

// Set pitch for music
void RLSetMusicPitch(Music music, float pitch)
{
    RLSetAudioBufferPitch(music.stream.buffer, pitch);
}

// Set pan for a music
void RLSetMusicPan(Music music, float pan)
{
    RLSetAudioBufferPan(music.stream.buffer, pan);
}

// Get music time length (in seconds)
float RLGetMusicTimeLength(Music music)
{
...
float RLGetMusicTimePlayed(Music music)
{
...
AudioStream RLLoadAudioStream(unsigned int sampleRate, unsigned int sampleSize, unsigned int channels)
{
    ...
    stream.buffer = RLLoadAudioBuffer(formatIn, stream.channels, stream.sampleRate, subBufferSize*2, AUDIO_BUFFER_USAGE_STREAM);
...
bool RLIsAudioStreamValid(AudioStream stream)
{
...
void RLUnloadAudioStream(AudioStream stream)
{
    RLUnloadAudioBuffer(stream.buffer);
...
void RLUpdateAudioStream(AudioStream stream, const void *data, int frameCount)
{
    ma_mutex_lock(&AUDIO.System.lock);
    RLUpdateAudioStreamInLockedState(stream, data, frameCount);
    ma_mutex_unlock(&AUDIO.System.lock);
}

// Check if any audio stream buffers requires refill
bool RLIsAudioStreamProcessed(AudioStream stream)
{
...
// Play audio stream
void RLPlayAudioStream(AudioStream stream)
{
    RLPlayAudioBuffer(stream.buffer);
}

// Play audio stream
void RLPauseAudioStream(AudioStream stream)
{
    RLPauseAudioBuffer(stream.buffer);
}

// Resume audio stream playing
void RLResumeAudioStream(AudioStream stream)
{
    RLResumeAudioBuffer(stream.buffer);
}

// Check if audio stream is playing
bool RLIsAudioStreamPlaying(AudioStream stream)
{
    return RLIsAudioBufferPlaying(stream.buffer);
}

// Stop audio stream
void RLStopAudioStream(AudioStream stream)
{
    RLStopAudioBuffer(stream.buffer);
}

// Set volume for audio stream (1.0 is max level)
void RLSetAudioStreamVolume(AudioStream stream, float volume)
{
    RLSetAudioBufferVolume(stream.buffer, volume);
}

// Set pitch for audio stream (1.0 is base level)
void RLSetAudioStreamPitch(AudioStream stream, float pitch)
{
    RLSetAudioBufferPitch(stream.buffer, pitch);
}

// Set pan for audio stream
void RLSetAudioStreamPan(AudioStream stream, float pan)
{
    RLSetAudioBufferPan(stream.buffer, pan);
}
...
void RLSetAudioStreamCallback(AudioStream stream, AudioCallback callback)
{
    if (stream.buffer != NULL)
    {
        ma_mutex_lock(&AUDIO.System.lock);
...
void RLAttachAudioStreamProcessor(AudioStream stream, AudioCallback process)
{
    ma_mutex_lock(&AUDIO.System.lock);
...
void RLDetachAudioStreamProcessor(AudioStream stream, AudioCallback process)
{
    ma_mutex_lock(&AUDIO.System.lock);
...
void RLAttachAudioMixedProcessor(AudioCallback process)
{
    ma_mutex_lock(&AUDIO.System.lock);
...
void RLDetachAudioMixedProcessor(AudioCallback process)
{
    ma_mutex_lock(&AUDIO.System.lock);
...
static ma_uint32 RLReadAudioBufferFramesInInternalFormat(AudioBuffer *audioBuffer, void *framesOut, ma_uint32 frameCount)
{
    ...
    while (1)
    {
        ...
        if (framesToRead == framesRemainingInOutputBuffer)
        {
            ...
            {
                RLStopAudioBufferInLockedState(audioBuffer);
                break;
            }
        }
    }
...
static ma_uint32 RLReadAudioBufferFramesInMixingFormat(AudioBuffer *audioBuffer, float *framesOut, ma_uint32 frameCount)
{
    ...
    while (totalOutputFramesProcessed < frameCount)
    {
        ...
        ma_uint64 inputFramesProcessedThisIteration = RLReadAudioBufferFramesInInternalFormat(audioBuffer, inputBuffer, (ma_uint32)inputFramesToProcessThisIteration);    /* Safe cast. */
        ...
        for (AudioBuffer *audioBuffer = AUDIO.Buffer.first; audioBuffer != NULL; audioBuffer = audioBuffer->next)
        {
            ...
            while (1)
            {
                ...
                while (framesToRead > 0)
                {
                    ...
                    ma_uint32 framesJustRead = RLReadAudioBufferFramesInMixingFormat(audioBuffer, tempBuffer, framesToReadRightNow);
                    if (framesJustRead > 0)
                    {
                        ...
                        RLMixAudioFrames(framesOut, framesIn, framesJustRead, audioBuffer);
...
static void RLStopAudioBufferInLockedState(AudioBuffer *buffer)
{
    if (buffer != NULL)
    {
        if (RLIsAudioBufferPlayingInLockedState(buffer))
        {
