...
//----------------------------------------------------------------------------------
extern void RLLoadFontDefault(void);          // [Module: text] Loads default font, required by ImageDrawText()

//----------------------------------------------------------------------------------
// Module specific Functions Declaration
//----------------------------------------------------------------------------------
static float RLHalfToFloat(unsigned short x);
static unsigned short RLFloatToHalf(float x);
static Vector4 * RLLoadImageDataNormalized(Image image);       // Load pixel data from image as Vector4 array (float normalized)
...
Image RLLoadImage(const char *fileName)
{
    ...
    if (fileData != NULL)
    {
        image = RLLoadImageFromMemory(RLGetFileExtension(fileName), fileData, dataSize);
...
Image RLLoadImageRaw(const char *fileName, int width, int height, int format, int headerSize)
{
    Image image = { 0 };
...
Image RLLoadImageAnim(const char *fileName, int *frames)
{
    Image image = { 0 };
    int frameCount = 0;

#if defined(SUPPORT_FILEFORMAT_GIF)
    if (RLIsFileExtension(fileName, ".gif"))
    {
        int dataSize = 0;
        unsigned char *fileData = RLLoadFileData(fileName, &dataSize);
...
//  - All frames are returned in RGBA format
//  - Frames delay data is discarded
Image RLLoadImageAnimFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int *frames)
{
    Image image = { 0 };
    int frameCount = 0;
    ...
    {
        image = RLLoadImageFromMemory(fileType, fileData, dataSize);
        frameCount = 1;
    }
...
Image RLLoadImageFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    Image image = { 0 };
...
Image RLLoadImageFromTexture(Texture2D texture)
{
    Image image = { 0 };
...
Image RLLoadImageFromScreen(void)
{
    Vector2 scale = RLGetWindowScaleDPI();
    Image image = { 0 };

    image.width = (int)(RLGetScreenWidth()*scale.x);
    image.height = (int)(RLGetScreenHeight()*scale.y);
    image.mipmaps = 1;
    image.format = PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
...
bool RLIsImageValid(Image image)
{
    bool result = false;
...
void RLUnloadImage(Image image)
{
    RL_FREE(image.data);
}
...
bool RLExportImage(Image image, const char *fileName)
{
    int result = 0;
    ...
    else
    {
        ...
        imgData = (unsigned char *)RLLoadImageColors(image);
        allocatedData = true;
    }

#if defined(SUPPORT_FILEFORMAT_PNG)
    if (RLIsFileExtension(fileName, ".png"))
    {
        int dataSize = 0;
        ...
        result = RLSaveFileData(fileName, fileData, dataSize);
        RL_FREE(fileData);
    }
#else
    if (false) { }
#endif
#if defined(SUPPORT_FILEFORMAT_BMP)
    else if (RLIsFileExtension(fileName, ".bmp")) result = stbi_write_bmp(fileName, image.width, image.height, channels, imgData);
#endif
#if defined(SUPPORT_FILEFORMAT_TGA)
    else if (RLIsFileExtension(fileName, ".tga")) result = stbi_write_tga(fileName, image.width, image.height, channels, imgData);
#endif
#if defined(SUPPORT_FILEFORMAT_JPG)
    else if (RLIsFileExtension(fileName, ".jpg") ||
        RLIsFileExtension(fileName, ".jpeg")) result = stbi_write_jpg(fileName, image.width, image.height, channels, imgData, 90);  // JPG quality: between 1 and 100
#endif
#if defined(SUPPORT_FILEFORMAT_QOI)
    else if (RLIsFileExtension(fileName, ".qoi"))
    {
        channels = 0;
...
#endif
#if defined(SUPPORT_FILEFORMAT_KTX)
    else if (RLIsFileExtension(fileName, ".ktx"))
    {
    ...
    else if (RLIsFileExtension(fileName, ".raw"))
    {
        ...
        result = RLSaveFileData(fileName, image.data, RLGetPixelDataSize(image.width, image.height, image.format));
    }
...
unsigned char * RLExportImageToMemory(Image image, const char *fileType, int *dataSize)
{
...
bool RLExportImageAsCode(Image image, const char *fileName)
{
    bool success = false;
    ...
    int dataSize = RLGetPixelDataSize(image.width, image.height, image.format);
...
Image RLGenImageColor(int width, int height, Color color)
{
...
Image RLGenImageGradientLinear(int width, int height, int direction, Color start, Color end)
{
...
Image RLGenImageGradientRadial(int width, int height, float density, Color inner, Color outer)
{
    Color *pixels = (Color *)RL_MALLOC(width*height*sizeof(Color));
...
Image RLGenImageGradientSquare(int width, int height, float density, Color inner, Color outer)
{
    Color *pixels = (Color *)RL_MALLOC(width*height*sizeof(Color));
...
Image RLGenImageChecked(int width, int height, int checksX, int checksY, Color col1, Color col2)
{
    Color *pixels = (Color *)RL_MALLOC(width*height*sizeof(Color));
...
Image RLGenImageWhiteNoise(int width, int height, float factor)
{
    Color *pixels = (Color *)RL_MALLOC(width*height*sizeof(Color));

    for (int i = 0; i < width*height; i++)
    {
        if (RLGetRandomValue(0, 99) < (int)(factor*100.0f)) pixels[i] = WHITE;
        else pixels[i] = BLACK;
    }
...
Image RLGenImagePerlinNoise(int width, int height, int offsetX, int offsetY, float scale)
{
    Color *pixels = (Color *)RL_MALLOC(width*height*sizeof(Color));
...
Image RLGenImageCellular(int width, int height, int tileSize)
{
    Color *pixels = (Color *)RL_MALLOC(width*height*sizeof(Color));
    ...
    {
        int y = (i/seedsPerRow)*tileSize + RLGetRandomValue(0, tileSize - 1);
        int x = (i%seedsPerRow)*tileSize + RLGetRandomValue(0, tileSize - 1);
...
Image RLGenImageText(int width, int height, const char *text)
{
    Image image = { 0 };
...
Image RLImageCopy(Image image)
{
    ...
    {
        size += RLGetPixelDataSize(width, height, image.format);
...
Image RLImageFromImage(Image image, Rectangle rec)
{
    Image result = { 0 };
...
void RLImageCrop(Image *image, Rectangle crop)
{
...
void RLImageFormat(Image *image, int newFormat)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
    ...
    if ((newFormat != 0) && (image->format != newFormat))
    {
        ...
        {
            Vector4 *pixels = RLLoadImageDataNormalized(*image);     // Supports 8 to 32 bit per channel
            ...
            switch (image->format)
            {
                ...
                case PIXELFORMAT_UNCOMPRESSED_R16:
                {
                    ...
                    for (int i = 0; i < image->width*image->height; i++)
                    {
                        ((unsigned short *)image->data)[i] = RLFloatToHalf((float)(pixels[i].x*0.299f + pixels[i].y*0.587f + pixels[i].z*0.114f));
                    }
                } break;
                case PIXELFORMAT_UNCOMPRESSED_R16G16B16:
                {
                    ...
                    for (int i = 0, k = 0; i < image->width*image->height*3; i += 3, k++)
                    {
                        ((unsigned short *)image->data)[i] = RLFloatToHalf(pixels[k].x);
                        ((unsigned short *)image->data)[i + 1] = RLFloatToHalf(pixels[k].y);
                        ((unsigned short *)image->data)[i + 2] = RLFloatToHalf(pixels[k].z);
                    }
                } break;
                case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
                {
                    ...
                    for (int i = 0, k = 0; i < image->width*image->height*4; i += 4, k++)
                    {
                        ...
                        ((unsigned short *)image->data)[i + 1] = RLFloatToHalf(pixels[k].y);
                        ((unsigned short *)image->data)[i + 2] = RLFloatToHalf(pixels[k].z);
                        ((unsigned short *)image->data)[i + 3] = RLFloatToHalf(pixels[k].w);
                    }
                } break;
                default: break;
            }
...
Image RLImageText(const char *text, int fontSize, Color color)
{
    Image imText = { 0 };
    ...
    imText = RLImageTextEx(RLGetFontDefault(), text, (float)fontSize, (float)spacing, color);   // WARNING: Module required: rtext
#else
...
Image RLImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
    Image imText = { 0 };
#if defined(SUPPORT_MODULE_RTEXT)
    ...
    Vector2 imSize = RLMeasureTextEx(font, text, (float)font.baseSize, spacing);  // WARNING: Module required: rtext
    Vector2 textSize = RLMeasureTextEx(font, text, fontSize, spacing);

    // Create image to store text
    imText = RLGenImageColor((int)imSize.x, (int)imSize.y, BLANK);

    for (int i = 0; i < size;)
    {
        // Get next codepoint from byte string and glyph index in font
        int codepointByteCount = 0;
        int codepoint = RLGetCodepointNext(&text[i], &codepointByteCount);    // WARNING: Module required: rtext
        int index = RLGetGlyphIndex(font, codepoint);                         // WARNING: Module required: rtext
        ...
        else
        {
            if ((codepoint != ' ') && (codepoint != '\t'))
            {
                ...
                RLImageDraw(&imText, font.glyphs[index].image, (Rectangle){ 0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height }, rec, tint);
            }
    ...
    if (textSize.y != imSize.y)
    {
        ...
        if (font.texture.id == RLGetFontDefault().texture.id) RLImageResizeNN(&imText, (int)(imSize.x*scaleFactor), (int)(imSize.y*scaleFactor));
        else RLImageResize(&imText, (int)(imSize.x*scaleFactor), (int)(imSize.y*scaleFactor));
    }
#else
    imText = RLGenImageColor(200, 60, BLACK);     // Generating placeholder black image rectangle
    TRACELOG(LOG_WARNING, "IMAGE: ImageTextEx() requires module: rtext");
#endif
    return imText;
}
...
Image RLImageFromChannel(Image image, int selectedChannel)
{
    Image result = { 0 };
    ...
    else
    {
        for (int i = 0, k = 0; i < image.width*image.height; i++)
        {
            ...
            switch (image.format)
            {
                ...
                case PIXELFORMAT_UNCOMPRESSED_R16:
                {
                    pixelValue = RLHalfToFloat(((unsigned short *)image.data)[k]);
                    k += 1;

                } break;
                case PIXELFORMAT_UNCOMPRESSED_R16G16B16:
                {
                    pixelValue = RLHalfToFloat(((unsigned short *)image.data)[k+selectedChannel]);
                    k += 3;
...
void RLImageResizeNN(Image *image,int newWidth,int newHeight)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;

    Color *pixels = RLLoadImageColors(*image);
...
void RLImageResize(Image *image, int newWidth, int newHeight)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageResizeCanvas(Image *image, int newWidth, int newHeight, int offsetX, int offsetY, Color fill)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
    ...
    else if ((newWidth != image->width) || (newHeight != image->height))
    {
        ...
        RLSetPixelColor(resizedData, fill, image->format);
...
void RLImageToPOT(Image *image, Color fill)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
    ...
    if ((potWidth != image->width) || (potHeight != image->height)) RLImageResizeCanvas(image, potWidth, potHeight, 0, 0, fill);
}

// Crop image depending on alpha value
// NOTE: Threshold is defined as a percentage: 0.0f -> 1.0f
void RLImageAlphaCrop(Image *image, float threshold)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;

    Rectangle crop = RLGetImageAlphaBorder(*image, threshold);

    // Crop if rectangle is valid
    if (((int)crop.width != 0) && ((int)crop.height != 0)) RLImageCrop(image, crop);
}

// Clear alpha channel to desired color
// NOTE: Threshold defines the alpha limit, 0.0f to 1.0f
void RLImageAlphaClear(Image *image, Color color, float threshold)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
    ...
    else
    {
        switch (image->format)
        {
            ...
            case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
            {
                for (int i = 3; i < image->width*image->height*4; i += 4)
                {
                    if (RLHalfToFloat(((unsigned short *)image->data)[i]) <= threshold)
                    {
                        ((unsigned short *)image->data)[i - 3] = RLFloatToHalf((float)color.r/255.0f);
                        ((unsigned short *)image->data)[i - 2] = RLFloatToHalf((float)color.g/255.0f);
                        ((unsigned short *)image->data)[i - 1] = RLFloatToHalf((float)color.b/255.0f);
                        ((unsigned short *)image->data)[i] = RLFloatToHalf((float)color.a/255.0f);
                    }
                }
            } break;
            default: break;
...
void RLImageAlphaMask(Image *image, Image alphaMask)
{
    ...
    else
    {
        ...
        Image mask = RLImageCopy(alphaMask);
        if (mask.format != PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) RLImageFormat(&mask, PIXELFORMAT_UNCOMPRESSED_GRAYSCALE);
        ...
        else
        {
            ...
            if (image->format != PIXELFORMAT_UNCOMPRESSED_R8G8B8A8) RLImageFormat(image, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8);
...
void RLImageAlphaPremultiply(Image *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageBlurGaussian(Image *image, int blurSize)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;

    RLImageAlphaPremultiply(image);
...
void RLImageKernelConvolution(Image *image, const float *kernel, int kernelSize)
{
...
void RLImageMipmaps(Image *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
    ...
    int mipSize = RLGetPixelDataSize(mipWidth, mipHeight, image->format);  // Image data size (in bytes)
    ...
    while ((mipWidth != 1) || (mipHeight != 1))
    {
        ...
        mipCount++;
        mipSize += RLGetPixelDataSize(mipWidth, mipHeight, image->format);       // Add mipmap size (in bytes)
    }
    ...
    if (image->mipmaps < mipCount)
    {
        ...
        mipSize = RLGetPixelDataSize(mipWidth, mipHeight, image->format);
        Image imCopy = RLImageCopy(*image);
...
void RLImageDither(Image *image, int rBpp, int gBpp, int bBpp, int aBpp)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageFlipVertical(Image *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageFlipHorizontal(Image *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageRotate(Image *image, int degrees)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageRotateCW(Image *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageRotateCCW(Image *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorTint(Image *image, Color color)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorInvert(Image *image)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorGrayscale(Image *image)
{
    RLImageFormat(image, PIXELFORMAT_UNCOMPRESSED_GRAYSCALE);
}

// Modify image color: contrast
// NOTE: Contrast values between -100 and 100
void RLImageColorContrast(Image *image, float contrast)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorBrightness(Image *image, int brightness)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void RLImageColorReplace(Image *image, Color color, Color replace)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
Color * RLLoadImageColors(Image image)
{
    ...
    else
    {
        ...
        for (int i = 0, k = 0; i < image.width*image.height; i++)
        {
            switch (image.format)
            {
                ...
                case PIXELFORMAT_UNCOMPRESSED_R16:
                {
                    pixels[i].r = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[k])*255.0f);
                    pixels[i].g = 0;
                    pixels[i].b = 0;
                    pixels[i].a = 255;
                ...
                {
                    pixels[i].r = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[k])*255.0f);
                    pixels[i].g = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[k + 1])*255.0f);
                    pixels[i].b = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[k + 2])*255.0f);
                    pixels[i].a = 255;
                ...
                case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
                {
                    ...
                    pixels[i].g = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[k + 1])*255.0f);
                    pixels[i].b = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[k + 2])*255.0f);
                    pixels[i].a = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[k + 3])*255.0f);
...
Color * RLLoadImagePalette(Image image, int maxPaletteSize, int *colorCount)
{
...
void RLUnloadImageColors(Color *colors)
{
    RL_FREE(colors);
}

// Unload colors palette loaded with LoadImagePalette()
void RLUnloadImagePalette(Color *colors)
{
    RL_FREE(colors);
}
...
// NOTE: Threshold is defined as a percentage: 0.0f -> 1.0f
Rectangle RLGetImageAlphaBorder(Image image, float threshold)
{
    Rectangle crop = { 0 };
...
Color RLGetImageColor(Image image, int x, int y)
{
    Color color = { 0 };
    ...
    if ((x >=0) && (x < image.width) && (y >= 0) && (y < image.height))
    {
        switch (image.format)
        {
            ...
            case PIXELFORMAT_UNCOMPRESSED_R16:
            {
                color.r = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[y*image.width + x])*255.0f);
                color.g = 0;
                color.b = 0;
                color.a = 255;
            ...
            case PIXELFORMAT_UNCOMPRESSED_R16G16B16:
            {
                color.r = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[(y*image.width + x)*3])*255.0f);
                color.g = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[(y*image.width + x)*3 + 1])*255.0f);
                color.b = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[(y*image.width + x)*3 + 2])*255.0f);
                color.a = 255;

            } break;
            case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
            {
                color.r = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[(y*image.width + x)*4])*255.0f);
                color.g = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[(y*image.width + x)*4])*255.0f);
                color.b = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[(y*image.width + x)*4])*255.0f);
                color.a = (unsigned char)(RLHalfToFloat(((unsigned short *)image.data)[(y*image.width + x)*4])*255.0f);
...
void RLImageClearBackground(Image *dst, Color color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0)) return;

    // Fill in first pixel based on image format
    RLImageDrawPixel(dst, 0, 0, color);

    unsigned char *pSrcPixel = (unsigned char *)dst->data;
    int bytesPerPixel = RLGetPixelDataSize(1, 1, dst->format);
...
void RLImageDrawPixel(Image *dst, int x, int y, Color color)
{
    // Security check to avoid program crash
    ...
    switch (dst->format)
    {
        ...
        case PIXELFORMAT_UNCOMPRESSED_R16:
        {
            ...
            ((unsigned short*)dst->data)[y*dst->width + x] = RLFloatToHalf(coln.x*0.299f + coln.y*0.587f + coln.z*0.114f);
        ...
        case PIXELFORMAT_UNCOMPRESSED_R16G16B16:
        {
            ...
            ((unsigned short *)dst->data)[(y*dst->width + x)*3] = RLFloatToHalf(coln.x);
            ((unsigned short *)dst->data)[(y*dst->width + x)*3 + 1] = RLFloatToHalf(coln.y);
            ((unsigned short *)dst->data)[(y*dst->width + x)*3 + 2] = RLFloatToHalf(coln.z);
        } break;
        case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
        {
            // NOTE: Calculate R32G32B32A32 equivalent color (normalized to 32bit)
            ...
            ((unsigned short *)dst->data)[(y*dst->width + x)*4] = RLFloatToHalf(coln.x);
            ((unsigned short *)dst->data)[(y*dst->width + x)*4 + 1] = RLFloatToHalf(coln.y);
            ((unsigned short *)dst->data)[(y*dst->width + x)*4 + 2] = RLFloatToHalf(coln.z);
            ((unsigned short *)dst->data)[(y*dst->width + x)*4 + 3] = RLFloatToHalf(coln.w);
...
void RLImageDrawPixelV(Image *dst, Vector2 position, Color color)
{
    RLImageDrawPixel(dst, (int)position.x, (int)position.y, color);
}

// Draw line within an image
void RLImageDrawLine(Image *dst, int startPosX, int startPosY, int endPosX, int endPosY, Color color)
{
    ...
    if (yLonger)
    {
        ...
        for (int i = 0, j = 0; i != endVal; i += sgnInc, j += decInc)
        {
            ...
            RLImageDrawPixel(dst, startPosX + (j >> 16), startPosY + i, color);
        }
    }
    else
    {
        ...
        {
            // Calculate pixel position and draw it
            RLImageDrawPixel(dst, startPosX + i, startPosY + (j >> 16), color);
        }
    }
}

// Draw line within an image (Vector version)
void RLImageDrawLineV(Image *dst, Vector2 start, Vector2 end, Color color)
{
    ...
    RLImageDrawLine(dst, x1, y1, x2, y2, color);
}

// Draw a line defining thickness within an image
void RLImageDrawLineEx(Image *dst, Vector2 start, Vector2 end, int thick, Color color)
{
    // Round start and end positions to nearest integer coordinates
    int x1 = (int)(start.x + 0.5f);
    int y1 = (int)(start.y + 0.5f);
    ...
    if (dx != 0 && abs(dy/dx) < 1)
    {
        ...
        for (int i = 1; i <= wy; i++)
        {
            RLImageDrawLine(dst, x1, y1 - i, x2, y2 - i, color); // Draw above the main line
            RLImageDrawLine(dst, x1, y1 + i, x2, y2 + i, color); // Draw below the main line
        }
    }
    else if (dy != 0)
    {
        ...
        for (int i = 1; i <= wx; i++)
        {
            RLImageDrawLine(dst, x1 - i, y1, x2 - i, y2, color); // Draw left of the main line
            RLImageDrawLine(dst, x1 + i, y1, x2 + i, y2, color); // Draw right of the main line
        }
    }
}

// Draw circle within an image
void RLImageDrawCircle(Image* dst, int centerX, int centerY, int radius, Color color)
{
    int x = 0;
    int y = radius;
    ...
    while (y >= x)
    {
        RLImageDrawRectangle(dst, centerX - x, centerY + y, x*2, 1, color);
        RLImageDrawRectangle(dst, centerX - x, centerY - y, x*2, 1, color);
        RLImageDrawRectangle(dst, centerX - y, centerY + x, y*2, 1, color);
        RLImageDrawRectangle(dst, centerX - y, centerY - x, y*2, 1, color);
        x++;
...
void RLImageDrawCircleV(Image* dst, Vector2 center, int radius, Color color)
{
    RLImageDrawCircle(dst, (int)center.x, (int)center.y, radius, color);
}

// Draw circle outline within an image
void RLImageDrawCircleLines(Image *dst, int centerX, int centerY, int radius, Color color)
{
    int x = 0;
    int y = radius;
    int decesionParameter = 3 - 2*radius;
    ...
    while (y >= x)
    {
        RLImageDrawPixel(dst, centerX + x, centerY + y, color);
        RLImageDrawPixel(dst, centerX - x, centerY + y, color);
        RLImageDrawPixel(dst, centerX + x, centerY - y, color);
        RLImageDrawPixel(dst, centerX - x, centerY - y, color);
        RLImageDrawPixel(dst, centerX + y, centerY + x, color);
        RLImageDrawPixel(dst, centerX - y, centerY + x, color);
        RLImageDrawPixel(dst, centerX + y, centerY - x, color);
        RLImageDrawPixel(dst, centerX - y, centerY - x, color);
        x++;
...
void RLImageDrawCircleLinesV(Image *dst, Vector2 center, int radius, Color color)
{
    RLImageDrawCircleLines(dst, (int)center.x, (int)center.y, radius, color);
}

// Draw rectangle within an image
void RLImageDrawRectangle(Image *dst, int posX, int posY, int width, int height, Color color)
{
    RLImageDrawRectangleRec(dst, (Rectangle){ (float)posX, (float)posY, (float)width, (float)height }, color);
}

// Draw rectangle within an image (Vector version)
void RLImageDrawRectangleV(Image *dst, Vector2 position, Vector2 size, Color color)
{
    RLImageDrawRectangle(dst, (int)position.x, (int)position.y, (int)size.x, (int)size.y, color);
}

// Draw rectangle within an image
void RLImageDrawRectangleRec(Image *dst, Rectangle rec, Color color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0)) return;
    ...
    RLImageDrawPixel(dst, sx, sy, color);
...
void RLImageDrawRectangleLines(Image *dst, Rectangle rec, int thick, Color color)
{
    RLImageDrawRectangle(dst, (int)rec.x, (int)rec.y, (int)rec.width, thick, color);
    RLImageDrawRectangle(dst, (int)rec.x, (int)(rec.y + thick), thick, (int)(rec.height - thick*2), color);
    RLImageDrawRectangle(dst, (int)(rec.x + rec.width - thick), (int)(rec.y + thick), thick, (int)(rec.height - thick*2), color);
    RLImageDrawRectangle(dst, (int)rec.x, (int)(rec.y + rec.height - thick), (int)rec.width, thick, color);
}

// Draw triangle within an image
void RLImageDrawTriangle(Image *dst, Vector2 v1, Vector2 v2, Vector2 v3, Color color)
{
    ...
    for (int y = yMin; y <= yMax; y++)
    {
        ...
        for (int x = xMin; x <= xMax; x++)
        {
            ...
            if ((w1 | w2 | w3) >= 0) RLImageDrawPixel(dst, x, y, color);
...
void RLImageDrawTriangleEx(Image *dst, Vector2 v1, Vector2 v2, Vector2 v3, Color c1, Color c2, Color c3)
{
    // Calculate the 2D bounding box of the triangle
    // Determine the minimum and maximum x and y coordinates of the triangle vertices
    int xMin = (int)((v1.x < v2.x)? ((v1.x < v3.x)? v1.x : v3.x) : ((v2.x < v3.x)? v2.x : v3.x));
    ...
    for (int y = yMin; y <= yMax; y++)
    {
        ...
        for (int x = xMin; x <= xMax; x++)
        {
            ...
            if ((w1 | w2 | w3) >= 0)
            {
                ...
                RLImageDrawPixel(dst, x, y, finalColor);
            }
...
void RLImageDrawTriangleLines(Image *dst, Vector2 v1, Vector2 v2, Vector2 v3, Color color)
{
    RLImageDrawLine(dst, (int)v1.x, (int)v1.y, (int)v2.x, (int)v2.y, color);
    RLImageDrawLine(dst, (int)v2.x, (int)v2.y, (int)v3.x, (int)v3.y, color);
    RLImageDrawLine(dst, (int)v3.x, (int)v3.y, (int)v1.x, (int)v1.y, color);
}

// Draw a triangle fan defined by points within an image (first vertex is the center)
void RLImageDrawTriangleFan(Image *dst, Vector2 *points, int pointCount, Color color)
{
    if (pointCount >= 3)
    {
        for (int i = 1; i < pointCount - 1; i++)
        {
            RLImageDrawTriangle(dst, points[0], points[i], points[i + 1], color);
        }
    }
}

// Draw a triangle strip defined by points within an image
void RLImageDrawTriangleStrip(Image *dst, Vector2 *points, int pointCount, Color color)
{
    if (pointCount >= 3)
    {
        for (int i = 2; i < pointCount; i++)
        {
            if ((i%2) == 0) RLImageDrawTriangle(dst, points[i], points[i - 2], points[i - 1], color);
            else RLImageDrawTriangle(dst, points[i], points[i - 1], points[i - 2], color);
        }
    }
}
...
void RLImageDraw(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, Color tint)
{
    // Security check to avoid program crash
    ...
    else
    {
        ...
        {
            srcMod = RLImageFromImage(src, srcRec);   // Create image from another image
            RLImageResize(&srcMod, (int)dstRec.width, (int)dstRec.height);   // Resize to destination rectangle
        ...
        int strideDst = RLGetPixelDataSize(dst->width, 1, dst->format);
        int bytesPerPixelDst = strideDst/(dst->width);

        int strideSrc = RLGetPixelDataSize(srcPtr->width, 1, srcPtr->format);
        ...
        for (int y = 0; y < (int)srcRec.height; y++)
        {
            ...
            else
            {
                ...
                {
                    colSrc = RLGetPixelColor(pSrc, srcPtr->format);
                    colDst = RLGetPixelColor(pDst, dst->format);

                    // Fast path: Avoid blend if source has no alpha to blend
                    if (blendRequired) blend = RLColorAlphaBlend(colDst, colSrc, tint);
                    else blend = colSrc;

                    RLSetPixelColor(pDst, blend, dst->format);
        ...
        if (useSrcMod) RLUnloadImage(srcMod);     // Unload source modified image

        if ((dst->mipmaps > 1) && (src.mipmaps > 1))
        {
            Image mipmapDst = *dst;
            mipmapDst.data = (char *)mipmapDst.data + RLGetPixelDataSize(mipmapDst.width, mipmapDst.height, mipmapDst.format);
            mipmapDst.width /= 2;
            mipmapDst.height /= 2;
            mipmapDst.mipmaps--;

            Image mipmapSrc = src;
            mipmapSrc.data = (char *)mipmapSrc.data + RLGetPixelDataSize(mipmapSrc.width, mipmapSrc.height, mipmapSrc.format);
            mipmapSrc.width /= 2;
            mipmapSrc.height /= 2;
            mipmapSrc.mipmaps--;
            ...
            RLImageDraw(&mipmapDst, mipmapSrc, mipmapSrcRec, mipmapDstRec, tint);
        }
    }
}

// Draw text (default font) within an image (destination)
void RLImageDrawText(Image *dst, const char *text, int posX, int posY, int fontSize, Color color)
{
#if defined(SUPPORT_MODULE_RTEXT) && defined(SUPPORT_DEFAULT_FONT)
    // Make sure default font is loaded to be used on image text drawing
    if (RLGetFontDefault().texture.id == 0) RLLoadFontDefault();

    Vector2 position = { (float)posX, (float)posY };
    RLImageDrawTextEx(dst, RLGetFontDefault(), text, position, (float)fontSize, 1.0f, color);   // WARNING: Module required: rtext
#else
...
void RLImageDrawTextEx(Image *dst, Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
    Image imText = RLImageTextEx(font, text, fontSize, spacing, tint);
    ...
    RLImageDraw(dst, imText, srcRec, dstRec, WHITE);

    RLUnloadImage(imText);
}
...
Texture2D RLLoadTexture(const char *fileName)
{
    Texture2D texture = { 0 };
...
Texture2D RLLoadTextureFromImage(Image image)
{
    Texture2D texture = { 0 };
...
TextureCubemap RLLoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        {
            faces = RLImageCopy(image);       // Image data already follows expected convention
        }
        ...
        else
        {
            ...
            faces = RLGenImageColor(size, size*6, MAGENTA);
            RLImageFormat(&faces, image.format);

            Image mipmapped = RLImageCopy(image);
            RLImageMipmaps(&mipmapped);
            RLImageMipmaps(&faces);

            // NOTE: Image formatting does not work with compressed textures

            for (int i = 0; i < 6; i++) RLImageDraw(&faces, mipmapped, faceRecs[i], (Rectangle){ 0, (float)size*i, (float)size, (float)size }, WHITE);

            RLUnloadImage(mipmapped);
        }
...
bool RLIsTextureValid(Texture2D texture)
{
    bool result = false;
...
void RLUnloadTexture(Texture2D texture)
{
    if (texture.id > 0)
    {
...
bool RLIsRenderTextureValid(RenderTexture2D target)
{
    bool result = false;

    if ((target.id > 0) &&                  // Validate OpenGL id (loaded on GPU)
        RLIsTextureValid(target.depth) &&     // Validate FBO depth texture/renderbuffer attachment
        RLIsTextureValid(target.texture)) result = true; // Validate FBO texture attachment

    return result;
}

// Unload render texture from GPU memory (VRAM)
void RLUnloadRenderTexture(RenderTexture2D target)
{
    if (target.id > 0)
    {
...
void RLUpdateTexture(Texture2D texture, const void *pixels)
{
...
// NOTE: pixels data must match texture.format
void RLUpdateTextureRec(Texture2D texture, Rectangle rec, const void *pixels)
{
...
void RLGenTextureMipmaps(Texture2D *texture)
{
...
void RLSetTextureFilter(Texture2D texture, int filter)
{
    switch (filter)
    {
...
void RLSetTextureWrap(Texture2D texture, int wrap)
{
    switch (wrap)
    {
...
//------------------------------------------------------------------------------------
// Draw a texture
void RLDrawTexture(Texture2D texture, int posX, int posY, Color tint)
{
    RLDrawTextureEx(texture, (Vector2){ (float)posX, (float)posY }, 0.0f, 1.0f, tint);
}

// Draw a texture with position defined as Vector2
void RLDrawTextureV(Texture2D texture, Vector2 position, Color tint)
{
    RLDrawTextureEx(texture, position, 0, 1.0f, tint);
}

// Draw a texture with extended parameters
void RLDrawTextureEx(Texture2D texture, Vector2 position, float rotation, float scale, Color tint)
{
    Rectangle source = { 0.0f, 0.0f, (float)texture.width, (float)texture.height };
    Rectangle dest = { position.x, position.y, (float)texture.width*scale, (float)texture.height*scale };
    Vector2 origin = { 0.0f, 0.0f };

    RLDrawTexturePro(texture, source, dest, origin, rotation, tint);
}

// Draw a part of a texture (defined by a rectangle)
void RLDrawTextureRec(Texture2D texture, Rectangle source, Vector2 position, Color tint)
{
    Rectangle dest = { position.x, position.y, fabsf(source.width), fabsf(source.height) };
    Vector2 origin = { 0.0f, 0.0f };

    RLDrawTexturePro(texture, source, dest, origin, 0.0f, tint);
}

// Draw a part of a texture (defined by a rectangle) with 'pro' parameters
// NOTE: origin is relative to destination rectangle size
void RLDrawTexturePro(Texture2D texture, Rectangle source, Rectangle dest, Vector2 origin, float rotation, Color tint)
{
...
void RLDrawTextureNPatch(Texture2D texture, NPatchInfo nPatchInfo, Rectangle dest, Vector2 origin, float rotation, Color tint)
{
    if (texture.id > 0)
    {
        float width = (float)texture.width;
...
bool RLColorIsEqual(Color col1, Color col2)
{
    bool result = false;
...
Color RLFade(Color color, float alpha)
{
    Color result = color;
...
Vector4 RLColorNormalize(Color color)
{
    Vector4 result;
...
Color RLColorFromNormalized(Vector4 normalized)
{
    Color result;
...
Vector3 RLColorToHSV(Color color)
{
    Vector3 hsv = { 0 };
...
Color RLColorFromHSV(float hue, float saturation, float value)
{
...
Color RLColorTint(Color color, Color tint)
{
    Color result = color;
...
Color RLColorBrightness(Color color, float factor)
{
    Color result = color;
...
Color RLColorContrast(Color color, float contrast)
{
    Color result = color;
...
// Get color with alpha applied, alpha goes from 0.0f to 1.0f
Color RLColorAlpha(Color color, float alpha)
{
    Color result = color;
...
Color RLColorAlphaBlend(Color dst, Color src, Color tint)
{
    Color out = WHITE;
...
Color RLColorLerp(Color color1, Color color2, float factor)
{
    Color color = { 0 };
...
Color RLGetColor(unsigned int hexValue)
{
    Color color;
...
Color RLGetPixelColor(void *srcPtr, int format)
{
    Color color = { 0 };
    ...
    switch (format)
    {
        ...
        {
            // NOTE: Pixel normalized float value is converted to [0..255]
            color.r = (unsigned char)(RLHalfToFloat(((unsigned short *)srcPtr)[0])*255.0f);
            color.g = (unsigned char)(RLHalfToFloat(((unsigned short *)srcPtr)[0])*255.0f);
            color.b = (unsigned char)(RLHalfToFloat(((unsigned short *)srcPtr)[0])*255.0f);
            color.a = 255;
        ...
        case PIXELFORMAT_UNCOMPRESSED_R16G16B16:
        {
            // NOTE: Pixel normalized float value is converted to [0..255]
            color.r = (unsigned char)(RLHalfToFloat(((unsigned short *)srcPtr)[0])*255.0f);
            color.g = (unsigned char)(RLHalfToFloat(((unsigned short *)srcPtr)[1])*255.0f);
            color.b = (unsigned char)(RLHalfToFloat(((unsigned short *)srcPtr)[2])*255.0f);
            color.a = 255;
        ...
        case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
        {
            ...
            color.g = (unsigned char)(RLHalfToFloat(((unsigned short *)srcPtr)[1])*255.0f);
            color.b = (unsigned char)(RLHalfToFloat(((unsigned short *)srcPtr)[2])*255.0f);
            color.a = (unsigned char)(RLHalfToFloat(((unsigned short *)srcPtr)[3])*255.0f);
...
void RLSetPixelColor(void *dstPtr, Color color, int format)
{
    switch (format)
    {
        case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
...
static Vector4 * RLLoadImageDataNormalized(Image image)
{
    ...
    else
    {
        for (int i = 0, k = 0; i < image.width*image.height; i++)
        {
            switch (image.format)
            {
                ...
                case PIXELFORMAT_UNCOMPRESSED_R16:
                {
                    pixels[i].x = RLHalfToFloat(((unsigned short *)image.data)[k]);
                    pixels[i].y = 0.0f;
                    pixels[i].z = 0.0f;
                    pixels[i].w = 1.0f;
                } break;
                ...
                {
                    pixels[i].x = RLHalfToFloat(((unsigned short *)image.data)[k]);
                    pixels[i].y = RLHalfToFloat(((unsigned short *)image.data)[k + 1]);
                    pixels[i].z = RLHalfToFloat(((unsigned short *)image.data)[k + 2]);
                    pixels[i].w = 1.0f;
                ...
                case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
                {
                    ...
                    pixels[i].y = RLHalfToFloat(((unsigned short *)image.data)[k + 1]);
                    pixels[i].z = RLHalfToFloat(((unsigned short *)image.data)[k + 2]);
                    pixels[i].w = RLHalfToFloat(((unsigned short *)image.data)[k + 3]);
