...
//----------------------------------------------------------------------------------
static float RLEaseCubicInOut(float t, float b, float c, float d);    // Cubic easing
...
void RLDrawLineBezier(Vector2 startPos, Vector2 endPos, float thick, Color color)
{
    ...
    for (int i = 1; i <= SPLINE_SEGMENT_DIVISIONS; i++)
    {
        ...
        current.y = RLEaseCubicInOut((float)i, startPos.y, endPos.y - startPos.y, (float)SPLINE_SEGMENT_DIVISIONS);
...
static float RLEaseCubicInOut(float t, float b, float c, float d)
{
    float result = 0.0f;
