...
public:
    ...
    static std::vector<ModelAnimation> Load(const std::string& fileName) {
        int count = 0;
        ::ModelAnimation* modelAnimations = ::RLLoadModelAnimations(fileName.c_str(), &count);
     ...
     */
    void Unload() { ::RLUnloadModelAnimation(*this); }

    /**
     * Update model animation pose
     */
    ModelAnimation& Update(const ::Model& model, int frame) {
        ::RLUpdateModelAnimation(model, *this, frame);
        return *this;
    }
    ...
    ModelAnimation& UpdateBones(const ::Model& model, int frame) {
        ::RLUpdateModelAnimationBones(model, *this, frame);
        return *this;
    }
     ...
     */
    bool IsValid(const ::Model& model) const { return ::RLIsModelAnimationValid(model, *this); }
protected:
