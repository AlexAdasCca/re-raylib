...
public:
     ...
     */
    Color(::Vector3 hsv) { set(::RLColorFromHSV(hsv.x, hsv.y, hsv.z)); }
     ...
     */
    Color(unsigned int hexValue) { set(::RLGetColor(hexValue)); }

    Color(void* srcPtr, int format) { set(::RLGetPixelColor(srcPtr, format)); }

    /**
     * Returns hexadecimal value for a Color
     */
    int ToInt() const { return ::RLColorToInt(*this); }

    /**
     * Returns hexadecimal value for a Color
     */
    operator int() const { return ::RLColorToInt(*this); }

    std::string ToString() const { return RLTextFormat("Color(%d, %d, %d, %d)", r, g, b, a); }
     ...
     */
    Color Fade(float alpha) const { return ::RLFade(*this, alpha); }

    /**
     * Returns Color normalized as float [0..1]
     */
    Vector4 Normalize() const { return ::RLColorNormalize(*this); }

    /**
     * Returns Color from normalized values [0..1]
     */
    Color(::Vector4 normalized) { set(::RLColorFromNormalized(normalized)); }

    /**
     * Returns HSV values for a Color
     */
    Vector3 ToHSV() const { return ::RLColorToHSV(*this); }
    ...
    Color& ClearBackground() {
        ::RLClearBackground(*this);
        return *this;
    }

    void DrawPixel(int x, int y) const { ::RLDrawPixel(x, y, *this); }

    /**
     * Draw a pixel
     */
    void DrawPixel(::Vector2 pos) const { ::RLDrawPixelV(pos, *this); }

    /**
     * Draw a line
     */
    void DrawLine(int startPosX, int startPosY, int endPosX, int endPosY) const {
        ::RLDrawLine(startPosX, startPosY, endPosX, endPosY, *this);
    }

    /**
     * Draw a line using Vector points
     */
    void DrawLine(::Vector2 startPos, ::Vector2 endPos) const { ::RLDrawLineV(startPos, endPos, *this); }

    /**
     * Draw a line using Vector points, with a given thickness
     */
    void DrawLine(::Vector2 startPos, ::Vector2 endPos, float thick) const {
        ::RLDrawLineEx(startPos, endPos, thick, *this);
    }

    void DrawLineBezier(::Vector2 startPos, ::Vector2 endPos, float thick = 1.0f) const {
        ::RLDrawLineBezier(startPos, endPos, thick, *this);
    }

    void DrawLineStrip(::Vector2* points, int numPoints) const { ::RLDrawLineStrip(points, numPoints, *this); }
    ...
    void DrawText(
        ...
        float fontSize,
        float spacing) const {
        ::RLDrawTextPro(font, text, position, origin, rotation, fontSize, spacing, *this);
    }
    ...
    void DrawText(
        ...
        float fontSize,
        float spacing) const {
        ::RLDrawTextPro(font, text.c_str(), position, origin, rotation, fontSize, spacing, *this);
    }

    void DrawRectangle(int posX, int posY, int width, int height) const {
        ::RLDrawRectangle(posX, posY, width, height, *this);
    }

    void DrawRectangle(::Vector2 position, ::Vector2 size) const { ::RLDrawRectangleV(position, size, *this); }

    void DrawRectangle(::Rectangle rec) const { ::RLDrawRectangleRec(rec, *this); }

    void DrawRectangle(::Rectangle rec, ::Vector2 origin, float rotation) const {
        ::RLDrawRectanglePro(rec, origin, rotation, *this);
    }

    void DrawRectangleLines(int posX, int posY, int width, int height) const {
        ::RLDrawRectangleLines(posX, posY, width, height, *this);
    }

    void DrawRectangleLines(::Rectangle rec, float lineThick) const { ::RLDrawRectangleLinesEx(rec, lineThick, *this); }

    bool IsEqual(::Color color) { return ::RLColorIsEqual(*this, color);
    }

    bool operator==(const ::Color& other) const { return ::RLColorIsEqual(*this, other); }
    bool operator!=(const ::Color& other) const { return !::RLColorIsEqual(*this, other); }

    /**
     * Get color multiplied with another color
     */
    Color Tint(::Color tint) { return ::RLColorTint(*this, tint); }

    /**
     * Get color with brightness correction, brightness factor goes from -1.0f to 1.0f
     */
    Color Brightness(float factor) { return ::RLColorBrightness(*this, factor); }

    /**
     * Get color with contrast correction, contrast values between -1.0f and 1.0f
     */
    Color Contrast(float contrast) { return ::RLColorContrast(*this, contrast); }

    /**
     * Returns color with alpha applied, alpha goes from 0.0f to 1.0f
     */
    Color Alpha(float alpha) const { return ::RLColorAlpha(*this, alpha); }

    Color Lerp(::Color color2, float factor) { return ::RLColorLerp(*this, color2, factor);
    }

    /**
     * Returns src alpha-blended into dst color with tint
     */
    Color AlphaBlend(::Color dst, ::Color tint) const { return ::RLColorAlphaBlend(dst, *this, tint); }
