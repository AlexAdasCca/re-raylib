...
public:
     ...
     */
    Material() { set(RLLoadMaterialDefault()); }
    ...
    static std::vector<Material> Load(const std::string& fileName) {
        ...
        ::Material* materials = ::RLLoadMaterials(fileName.c_str(), &count);
    ...
    Material& SetTexture(int mapType, const ::Texture2D& texture) {
        ::RLSetMaterialTexture(this, mapType, texture);
        return *this;
    }
     ...
     */
    void DrawMesh(const ::Mesh& mesh, ::Matrix transform) const { ::RLDrawMesh(mesh, *this, transform); }

    /**
     * Draw multiple mesh instances with material and different transforms
     */
    void DrawMesh(const ::Mesh& mesh, ::Matrix* transforms, int instances) const {
        ::RLDrawMeshInstanced(mesh, *this, transforms, instances);
    }

    /**
     * Check if material is ready
     */
    bool IsValid() const { return ::RLIsMaterialValid(*this); }
protected:
