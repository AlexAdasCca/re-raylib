...
public:
     ...
     * Unload music stream
     */
    void Unload() { ::RLUnloadMusicStream(*this); }

    /**
     * Start music playing
     */
    Music& Play() {
        ::RLPlayMusicStream(*this);
        return *this;
    }
    ...
    Music& Seek(float position) {
        RLSeekMusicStream(*this, position);
        return *this;
    }
     ...
     */
    bool IsPlaying() const { return ::RLIsMusicStreamPlaying(*this); }

    /**
     * Set volume for music
     */
    Music& SetVolume(float volume) {
        ::RLSetMusicVolume(*this, volume);
        return *this;
    }
     ...
     */
    float GetTimeLength() const { return ::RLGetMusicTimeLength(*this); }

    /**
     * Get current music time played (in seconds)
     */
    float GetTimePlayed() const { return ::RLGetMusicTimePlayed(*this); }
    ...
    void Load(const std::string& fileName) {
        set(::RLLoadMusicStream(fileName.c_str()));
        if (!IsValid()) {
            throw RaylibException(RLTextFormat("Failed to load Music from file: %s", fileName.c_str()));
        }
    }
    ...
    void Load(const std::string& fileType, unsigned char* data, int dataSize) {
        set(::RLLoadMusicStreamFromMemory(fileType.c_str(), data, dataSize));
        if (!IsValid()) {
            throw RaylibException(RLTextFormat("Failed to load Music from %s file dat", fileType.c_str()));
        }
    }
     ...
     */
    bool IsValid() const { return ::RLIsMusicValid(*this); }
protected:
