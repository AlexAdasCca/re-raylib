...
public:
    ...
    void Load(const std::string& fileName, int fontSize, int* fontChars, int charCount) {
        set(::RLLoadFontEx(fileName.c_str(), fontSize, fontChars, charCount));
        if (!IsValid()) {
    ...
    void Load(const ::Image& image, ::Color key, int firstChar) {
        set(::RLLoadFontFromImage(image, key, firstChar));
        if (!IsValid()) {
    ...
    void Load(
        ...
        int* fontChars,
        int charsCount) {
        set(::RLLoadFontFromMemory(fileType.c_str(), fileData, dataSize, fontSize, fontChars, charsCount));
        if (!IsValid()) {
     ...
     */
    bool IsValid() const { return ::RLIsFontValid(*this); }
    ...
    void DrawText(const char* text, int posX, int posY, float fontSize, float spacing, ::Color tint = WHITE) const {
        ::RLDrawTextEx(*this, text, {static_cast<float>(posX), static_cast<float>(posY)}, fontSize, spacing, tint);
    }
    ...
    void DrawText(
        ...
        ::RLDrawTextPro(*this, text, position, origin, rotation, fontSize, spacing, tint);
    }
    ...
    void DrawText(
        ...
        float spacing,
        ::Color tint = WHITE) const {
        ::RLDrawTextPro(*this, text.c_str(), position, origin, rotation, fontSize, spacing, tint);
    }
    ...
    void DrawText(int codepoint, ::Vector2 position, float fontSize, ::Color tint = {255, 255, 255, 255}) const {
        ::RLDrawTextCodepoint(*this, codepoint, position, fontSize, tint);
    }
    ...
    void DrawText(
        ...
        ::RLDrawTextCodepoints(*this, codepoints, count, position, fontSize, spacing, tint);
    }
    ...
    Vector2 MeasureText(const char* text, float fontSize, float spacing) const {
        return ::RLMeasureTextEx(*this, text, fontSize, spacing);
    }
    ...
    Vector2 MeasureText(const std::string& text, float fontSize, float spacing) const {
        return ::RLMeasureTextEx(*this, text.c_str(), fontSize, spacing);
    }

    /**
     * Get index position for a unicode character on font
     */
    int GetGlyphIndex(int character) const { return ::RLGetGlyphIndex(*this, character); }

    /**
     * Create an image from text (custom sprite font)
     */
    ::Image ImageText(const char* text, float fontSize, float spacing, ::Color tint) const {
        return ::RLImageTextEx(*this, text, fontSize, spacing, tint);
    }
    ...
    ::Image ImageText(const std::string& text, float fontSize, float spacing, ::Color tint) const {
        return ::RLImageTextEx(*this, text.c_str(), fontSize, spacing, tint);
    }
protected:
